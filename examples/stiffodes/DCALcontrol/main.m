...
% 3. Find partial derivatives of the ODE
% Calls ode15s and plots results
...
if supplyderiv
  ...
  options = odeset('Jacobian',@adigatorodewrapper,'Jpattern',S,'Stats','on','RelTol',sqrt(eps));
else
...
tic
[t,X] = ode15s(@TwoLinkSys,[0 tf],X0,options);
runtime = toc;
fprintf(['Simulation time = ',num2str(runtime),'\n']);

% options2 = odeset('Stats','on','RelTol',sqrt(eps));
% tic
% [t,X] = ode15s(@TwoLinkSys,[0 tf],X0,options2);
% runtime2 = toc
qd = getqd(t);
qd = qd.';
N = length(t);
...
if probinfo.noiseflag
   enoise = qd - q.*(1+interp1(0:.2:tf,probinfo.noise.',t)); 
end
...
if displayplots
figure(1);
plot(t,e(:,1)./(1+abs(qd(:,1))),'-');
xlabel('time (s)');
ylabel('link1 relative error');

figure(2)
plot(t,e(:,2)./(1+abs(qd(:,2))),'-');
xlabel('time (s)');
ylabel('link2 relative error');
