classdef ISymbolicIndexTest < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function sparseSymbolicIndexIsActionable(tc)
            % #121: @cada/sparse's five symbolic-index sites formerly raised
            % the pre-ADR-0024 cryptic 'cannot index with a strictly symbolic
            % variable' (no identifier); they now route through the shared
            % cadaErrorSymbolicIndex helper like subsref/subsasgn, so a
            % runtime-valued sparse() index gets the same actionable message
            % and pinnable id.
            writeRaw('symidx_sparse', { ...
                'function y = symidx_sparse(x)', ...
                'N = 3;', ...
                'n = 1; s = 0;', ...
                'while n <= N', ...
                '    S = sparse(n,1,2.5,4,1);', ...
                '    s = s + x(1);', ...
                '    n = n + 1;', ...
                'end', ...
                'y = s;', ...
            ...
            gx = adigatorCreateDerivInput([1 1],'x');
            try
                adigatorGenJacFile('symidx_sparse',{gx},struct('echo',0));
                tc.verifyFail(['a while-counter sparse() row index must raise ', ...
                    'the symbolic-index error']);
            catch err
                tc.verifyEqual(err.identifier, 'adigator:symbolicIndex', ...
                    'sparse() symbolic index must carry the ADR-0024 id');
                tc.verifySubstring(err.message, 'logical weights', ...
                    'error must point to the logical-weight-sum rewrite');
            end
        end

        function diagSymbolicIndexIsActionable(tc)
            % #121: same wiring for @cada/diag's symbolic diagonal-offset site
            % (formerly 'cannot assign to strictly symbolic diagonal', no id).
            writeRaw('symidx_diag', { ...
                'function y = symidx_diag(x)', ...
                'n = 1; s = 0;', ...
                'while n <= 2', ...
                '    D = diag([x(1); x(1)], n);', ...
                '    s = s + x(1);', ...
            ...
            gx = adigatorCreateDerivInput([1 1],'x');
            try
                adigatorGenJacFile('symidx_diag',{gx},struct('echo',0));
                tc.verifyFail(['a while-counter diag() offset must raise ', ...
                    'the symbolic-index error']);
            catch err
                tc.verifyEqual(err.identifier, 'adigator:symbolicIndex', ...
                    'diag() symbolic offset must carry the ADR-0024 id');
                tc.verifySubstring(err.message, 'logical weights', ...
                    'error must point to the logical-weight-sum rewrite');
            end
        end
