...
%
% Generates <UserFun>_RGrd.m computing [Grd, Fun] = <UserFun>_RGrd(inputs),
% where Grd is the column gradient with respect to the (single) derivative
% input and Fun the scalar value of the user function (DESIGN Contract C-6:
% derivative first, value last), by a REVERSE
...
%              output's adjoint seeded by a runtime input, the J'*v
%              mechanism of adigatorGenJtVFile; roadmap R5).
%   2026-06    Extract the statement parser / backward value-tape slicer to
%              adigatorForwardTapeSlice for reuse by the R7b field-slice
%              (issue #21); behaviour unchanged, error ids now
%              adigator:fwdtape:* (roadmap R7a follow-up).
%
% See also adigator adigatorGenJacFile adigatorGenJtVFile adigatorOptions
...
if isfield(opts,'filename') && ~isempty(opts.filename)
  RevName = char(opts.filename); % caller-chosen name (e.g. adigatorGenJtVFile)
else
...
% seed-input mode (adigatorGenJtVFile): the adjoint of a VECTOR output is
% seeded with a runtime vector v appended as the last input, producing the
% transposed-Jacobian-vector product J.'*v instead of a scalar gradient
if isfield(opts,'seedname') && ~isempty(opts.seedname)
  seedname = char(opts.seedname);
  if isempty(regexp(seedname,'^[A-Za-z]\w*$','once')) || ...
      any(strcmp(InNames,seedname))
    error('adigator:revgrad:inputs',...
      'invalid or colliding seed input name ''%s''',seedname);
  end
else
  seedname = '';
end
...
% --------- collect, parse, and backward-slice to the value tape -------- %
% The parser/slicer is shared with the R7b field-slice (issue #21); it was
% extracted verbatim into adigatorForwardTapeSlice (only the error-id prefix
% changed, adigator:fwdtape:*). It rejects rolled control flow and returns
% the value-tape statements, excluding the '.d<VodName>' derivative chain.
S = adigatorForwardTapeSlice(body, InNames, OutName, VodName);
n = numel(S);

% ------------------ execute the tape; classify; resolve ---------------- %
S = execAndClassify(S,FwdGator,UserFunInputs,InNames,vodLoc,VodName);
if isempty(seedname) && ~isequal(S(n).info.lsz,[1 1])
  error('adigator:revgrad:outputs',...
...
vinit = false;
  ...
  function t = selftext(k)
    % forward value of statement k's own result, as seen in reverse (the
    % output keeps its .f field until after the reverse sweep)
    if snap(k)
...
% ------------------------------ header --------------------------------- %
if isempty(seedname)
  hdrins = strjoin(InNames,',');
  derivOut = 'Grd';      % canonical gradient name (DESIGN Contract C-6)
else
  hdrins = [strjoin(InNames,','),',',seedname];
  derivOut = 'Jtv';      % canonical J.'*v product name (C-6)
end
valOut = 'Fun';          % canonical function-value name (C-6)
% C-6 wrapper-output order: derivative first, function value (Fun) last.
emit(sprintf('function [%s, %s] = %s(%s)',derivOut,valOut,RevName,hdrins));
if isempty(seedname)
  emit(['% ',RevName,' - reverse-mode (adjoint) gradient of ''',UserFun,'''']);
  emit(sprintf(['%% Returns [Grd, Fun]: the %d x 1 gradient w.r.t. %s, then the',...
    ' scalar value (C-6 order).'],prod(vodsize),VodName));
else
  emit(['% ',RevName,' - transposed-Jacobian-vector product of ''',...
    UserFun,'''']);
  emit(sprintf(['%% Returns [Jtv, Fun]: J.''*%s(:) (%d x 1) w.r.t. %s, then the',...
    ' function value (C-6 order).'],seedname,prod(vodsize),VodName));
end
...
if isempty(seedname)
  ...
  emit(sprintf('assert(numel(%s) == %d);',seedname,prod(S(n).info.lsz)));
  emit(sprintf('%s = reshape(%s,%d,%d);',bar{n},seedname,...
    S(n).info.lsz(1),S(n).info.lsz(2)));
end
init(n) = true;
...
% the tape writes the output as a struct field; return the plain value
% (converted only AFTER the reverse sweep, which still reads it)
emit(sprintf('%s = %s.f;',valOut,OutName));
emit(sprintf('%s = %s(:);',derivOut,vbar));
emit('end');
emit('');
emit('');
