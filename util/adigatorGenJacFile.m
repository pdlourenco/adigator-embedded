...
NameAppendix = 'Jac';
% #200: who to name in the generated file's "Reconstruct with:" line. Defaults
% to this function, but an outer entry point that post-processes what we write
% (adigatorGenDerFile_embedded) must be able to say so - naming ourselves for a
% file we did not finish producing would print a recipe that yields a different
% file. See adigatorReconstructCall.
ReconstructEntry = 'adigatorGenJacFile';
if nargin == 2
  opts.overwrite = 1;
else
    % parse options
...
fprintf(fid,'%% %s wrapper file generated by ADiGator\n', roleOf(NameAppendix));
fprintf(fid,'%% \n');
cadaPrintGeneratedHeader(fid, JacFileName, adigatorStampOf(FunctionInfo), ...
  adigatorReconstructCall(UserFunName, JacFileName, opts, ReconstructEntry, NameAppendix));
...
%% ---------------------------------------------------------------------- %%
function r = roleOf(nameAppendix)
% Human-readable role for the wrapper's own label line. adigatorGenHesFile
% labels its two wrappers by role (M4, #121, pinned by TS-I-12); this generator
% emits either a Jacobian or - when called with the 'Grd' appendix - a gradient,
% and had been left unlabelled, which is the asymmetry this restores.
switch nameAppendix
  case 'Jac'; r = 'Jacobian';
  case 'Grd'; r = 'Gradient';
  otherwise;  r = nameAppendix;
end
