...
% #200: see adigatorGenJacFile - an outer entry point that post-processes what we
% write must be able to name itself in the "Reconstruct with:" line.
ReconstructEntry = 'adigatorGenHesFile';
if nargin >= 4 && ~isempty(varargin{2})
    ReconstructEntry = varargin{2};
end
if nargin == 2
  opts.overwrite = 1;
else
...
% #200: one provenance stamp for the whole second-order run. The second adigator
% pass below differentiates the file the first pass just generated, so left to
% itself it would stamp from a closure containing our own output - giving
% <fn>_ADiGatorHes an id that differs from the <fn>_Hes wrapper calling it (a
% coherent generation reading as mixed vintages) and that changes on every run
% from identical sources (the generated file's header carries a wall clock).
% Take the first pass's stamp and hand it to the second.
HesStamp  = adigatorStampOf(FunctionInfo);
optsPass2 = opts;
optsPass2.inherited_stamp = HesStamp;
...
try % v2.0 - add try-catch to avoid leaving unnecessary changes to path active
[adiout2,~,ADi_DerivFiles2,ADi_DerivFuns2] = adigator(AdiGrdFileName,UserFunInputs,AdiHesFileName,optsPass2); % v2.0 - add new output with list of files/functions (FunctionInfo2 unused -> ~); optsPass2 carries the shared #200 stamp
catch ME
    path(original_path);
    rethrow(ME);
end
...
Hfuncstr = ['function [',strjoin(hesOut,','),'] = ',HesFileName,'(',InputStr1,')\n'];
  ...
  % Print Function Header (#200). Both wrappers carry the same generation stamp
  % as the derivative files they call - a mismatch across the set means mixed
  % vintages, which is the whole point of the id. HesStamp was taken from the
  % first pass and handed to the second, so all four files share it.
for fid = [Gfid Hfid]
  if fid == Hfid   % M4: the Hessian wrapper's header must carry the Hessian
                   % signature/label, not the gradient's (Hfuncstr was built but unused)
    fprintf(fid,['%% ',Hfuncstr,]);
    fprintf(fid,'%% \n');
    % Role label kept: M4 (#121) fixed this wrapper advertising the GRADIENT
    % label, and TS-I-12 pins it. The #200 header adds provenance; it does not
    % replace the role.
    fprintf(fid,'%% Hessian wrapper file generated by ADiGator\n');
    fprintf(fid,'%% \n');
    cadaPrintGeneratedHeader(fid, HesFileName, HesStamp, ...
      adigatorReconstructCall(UserFunName, HesFileName, opts, ReconstructEntry));
  else
    fprintf(fid,['%% ',Gfuncstr,]);
    fprintf(fid,'%% \n');
    fprintf(fid,'%% Gradient wrapper file generated by ADiGator\n');
    fprintf(fid,'%% \n');
    cadaPrintGeneratedHeader(fid, GrdFileName, HesStamp, ...
      adigatorReconstructCall(UserFunName, GrdFileName, opts, ReconstructEntry));
  end
end
