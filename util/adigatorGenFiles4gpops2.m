...
%
% see also adigator, gpops2, adigatorOptions
...
for p = 1:nPhases;
  ...
  if isfield(setup.bounds.phase,'control') && isfield(setup.bounds.phase(p).control,'lower') ...
      && ~isempty(setup.bounds.phase(p).control.lower);
    numcontrol(p) = size(setup.bounds.phase(p).control.lower,2);
  end
  if isfield(setup.bounds.phase,'integral') && isfield(setup.bounds.phase(p).integral,'lower') ...
      && ~isempty(setup.bounds.phase(p).integral.lower);
