...
% ...and, alongside each analytical reference, the FINITE-DIFFERENCE method
% (issue #73): the same central-difference derivative written Coder-compatibly
% (bench/showcase/fd/, via the fdDeriv kernel) so FD flows THROUGH ERT to a real
% compiled footprint like every other method - not just an interpreted-cost
% column. FD is the only INEXACT method, so its MEX-vs-analytic error is checked
% at a loose finite-difference tolerance (buildCell), not the 1e-9 AD/analytic one.
cells = [ mk('vcostfun','gradient',         1, 'AD',      ''), ...
          mk('vcostfun','gradient-reverse', 1, 'AD',      ''), ...
          mk('vcostfun','gradient',        -1, 'analytic','vcostfun_grad_analytic'), ...
          mk('vcostfun','gradient',        -1, 'FD',      'vcostfun_grad_fd'), ...
          mk('vcostfun','hessian',          1, 'AD',      ''), ...
          mk('vcostfun','hessian',         -1, 'analytic','vcostfun_hess_analytic'), ...
          mk('vcostfun','hessian',         -1, 'FD',      'vcostfun_hess_fd'), ...
          mk('vvecfun','jacobian',          1, 'AD',      ''), ...
          mk('vfun','jacobian',             0, 'AD',      ''), ...
          mk('vvecfun','jacobian',         -1, 'analytic','vvecfun_jac_analytic'), ...
          mk('vvecfun','jacobian',         -1, 'FD',      'vvecfun_jac_fd') ];
...
try
    ...
    % FD is inexact by design (central-difference truncation ~1e-8); check it at
    % a loose finite-difference tolerance so a within-tolerance FD build is 'ok'
    % and its error is reported, not failed. AD/analytical stay exact (1e-9).
    tol = ternval(strcmp(impl,'FD'), 1e-4, 1e-9);
    r.ok = r.mexErr <= tol*max(1,norm(ga(:),inf));
...
function v = ternval(c,a,b); if c; v=a; else; v=b; end; end
...
for k = 1:numel(rows)
    ...
    L(end+1) = string(sprintf("%s & %s & %s & %d & %s & %s \\\\", ...
        ...
        romStr, ratioStr(mr, isAna, '%.1f'))); %#ok<AGROW>
end
L(end+1) = "\hline";
L(end+1) = "\end{tabular}";
