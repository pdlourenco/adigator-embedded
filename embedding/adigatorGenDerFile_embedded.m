...
for derf = 1:N_derivs
    ...
    if selfContained
        ...
        % #200: the joined file carries ONE header - the wrapper's. The
        % derivative part was generated separately with its own full header;
        % demote it to a marker so the licence, disclaimer and provenance stamp
        % appear once for the file rather than once per part.
        writelines(adigatorDemoteGeneratedHeader(auxiliary_deriv_filecontents, ...
            'derivative body, generated by ADiGator (see the header above)'), ...
            AdigatorGeneratedFiles(derf).main,'WriteMode','append');
    end
    if inline
