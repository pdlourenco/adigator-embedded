...
- `.github/workflows/release.yml` — on a `v*` tag push it (1) **validates** the
  tag against `CHANGELOG.md`: the version must have a dated
  `## [X.Y] — YYYY-MM-DD` section and `[Unreleased]` must be empty at the tagged
  commit (mismatch → the run fails, no release); (2) **extracts** that version's
  section as the release body; (3) assembles a **curated runtime distribution**
  archive (the tagged tree minus test/bench/dev material — the tool, `examples/`,
  and the user guide, ready to `addpath`); (4) creates the GitHub release with
...
- The curated archive's exclusion list (`ARCHIVE_EXCLUDES`) is a tuning surface,
  not a contract; it currently strips `tests/`, `bench/`, internal docs, and the
  gitignored proprietary `docs/known-bugs`. `git archive HEAD` already omits
  untracked/gitignored files, so proprietary material cannot leak even if the
  list drifts.
