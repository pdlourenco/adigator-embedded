...
1. **Deterministic committed artifact.** The guide build runs plain `pdflatex`
   ...
   **no timestamps** — so re-running it on the same environment produces a
   byte-identical fragment when the measured inputs are unchanged (avoids the
   #115 `SOURCE_DATE_EPOCH` churn class). The rounding in constraint 2 keeps
   sub-precision measurement noise from breaking this.
2. **Machine-dependent figures are kept as a stamped snapshot, not omitted.**
   The MATLAB-generator metrics (code-lines, ROM/`.mat` bytes) are deterministic
   and emitted exact. The host/Coder-sensitive figures — generated-C size
   (MATLAB Coder / toolchain-version dependent, and a poor ROM proxy per
   `bench/SHOWCASE.md`) and wall-clock `timeit` runtime — are **included as
   absolute values alongside their ratio** (e.g. MEX-vs-interpreted), and the
   fragment is **captioned with the generating environment** — machine,
   MATLAB/Coder version, toolchain — exactly as `bench/SHOWCASE.md` already
   stamps its own tables. Because these figures vary run-to-run and across
   machines, the emitter (a) **rounds to a stable presentation precision** so
   sub-precision noise does not rewrite the committed fragment, and (b) the
   fragment is **refreshed deliberately** by the MATLAB session — the
   bench+Coder run is not part of the plain-`pdflatex` guide/CI build — so the
   numbers and their environment caption always move together. A regeneration on
   a new machine updating both is the intended, honest behavior: the guide shows
   real measured figures with the provenance to read them, not omitted numbers.
