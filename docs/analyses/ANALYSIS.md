...
level** (`DERNUMBER==2`) is a separate scope: a loopbound Hessian currently
**errors at generation** — the second-derivative pass cannot process the
gradient file's `assert(name<=max)` guard (`Cannot process statement:
assert(N <= 4)`) — rather than silently zeroing, so it is fail-loud
(principle-1-safe) in the basic case; whether a narrower reachable pattern zeroes
a second-derivative exit is under-characterized, tracked (with a `KnownIssue`
tripwire once characterized) by
[#173](https://github.com/pdlourenco/adigator-embedded/issues/173).
