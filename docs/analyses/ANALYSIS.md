...
execution falls into the non-vectorized path with the FIRST operand's `xNcol`
still `Inf`, and `true(xMrow, xNcol)` — `:158` when the operand carries
`zerolocs`, `:162` otherwise, which is the death site here — raises
`MATLAB:nonaninf`. (The `:168`/`:172` pair is the `y`-side twin and is *not*
where `x'*x` dies; for `x'` the `Inf` arrives through the left operand.) ("NaN and Inf not
...
`adigator:norm:matrixNorm` (`cada.m:667`) and B39's
...
Open. The fix — teach `mtimes` to guard the contracted dimension, and give the
vectorized refusals identifiers — belongs with vectorization work rather than on
its own, so both halves are held by a **self-healing tripwire**,
`tests/integration/IVectorizedFenceTest.m` (TS-I-31, license-free). It pins only
that the failure is *not ours* rather than pinning `MATLAB:nonaninf`, which is a
MATLAB-internal id and an artifact of `true(...,Inf)` — the unstable kind #226
refused to pin. Either fix landing makes it fire and say what to retire. #56 is *not* the right tracker — it is
