...
- Known bugs documented in `docs/ANALYSIS.md` (B1-B22) must be *pinned* by
  ...
  won't-fix / documented-limitation** — the open rough edges are the B19 `if`-guarded
  over-approximation residual (#108) and B40 (the vectorized refusal fence: one
  contraction escapes it, and none of its refusals carries an identifier), the
  latter held by the `IVectorizedFenceTest` self-healing tripwire (TS-I-31).
  Neither engages principle 1. The remaining `KnownIssue`-tagged tests are
...
| REQ-T-07 | **Robustness / hygiene.** Invalid inputs and mid-transformation errors shall raise clean errors, restore the MATLAB path, close all file handles, leave no stray globals, **and leave the session able to perform a subsequent generation correctly**. *Re-entry is necessary, not implied:* releasing state is strictly weaker than being usable again — an engine can restore `path()`, close every handle and clear every global and still be unable to start over. This is an already-accepted consequence of [ADR-0011](decisions/ADR-0011-adigator-error-path-cleanup.md) ("safe to call in a loop / test campaign — a failed transformation cannot poison later ones") that had never been stated as a requirement, and REQ-T-09's Monte-Carlo campaign has always depended on it: `MCSmokeTest.negativeHygieneIsClean` runs nine malformed generations back-to-back in one process and its other campaigns share that process, so without re-entry every case after the first negative would be suspect. | `path()` before == after; open-file-handle delta empty (`openedFiles()`, falling back to `fopen('all')`); `who('global')` delta empty after failure injection; **a generation following an injected failure succeeds and matches its analytic reference**, sampled at BOTH failure modes — one dying in the initial user-function evaluation (before adigator's handles open) and one dying inside the transformation pass (with them open). |
...
| REQ-T-10 | **Embedded-Coder (ERT) codegen completeness.** Every non-classic (`'l'`/`'i'`) generated derivative — gradient, Jacobian, Hessian, `gradient-reverse`, across `slim_embed` settings — shall codegen under the **stricter Embedded Coder target** — the single shared `adigatorCoderConfig()` (`coder.config('lib','ecoder',true)` **+ `EnableDynamicMemoryAllocation=false`**, so no `malloc` on the target), not only plain MATLAB Coder `lib` (which tolerates ERT-illegal struct-field patterns and was masking real embedded-codegen gaps). Every codegen site (`SCodegenTest`, `SRolledErtCodegenTest`, `oracleCodegenEquivalence`, `derivShowcaseC`, `loopboundPaddingPenalty`) consumes that one helper so the strict policy cannot drift across sites (issue #80). *(Core fork objective, issue #80; phased in `docs/ROADMAP.md` R20 — Gap A landed PR #81; Gap B rolled-path done via Path A (#89), with the unrolled form and reverse-mode deferred to R21. Distinct from REQ-T-05, which checks compiled-output equivalence; this checks ERT acceptance.)* | `ecoder` codegen exit-success for every (DerType × `'l'`/`'i'` mode × `slim_embed`) cell exercised by the showcase / campaign; no plain-`lib`-only pass left masking an ERT failure. **Exit-success is necessary, not sufficient:** a *bounded-but-large* derivative code-generates but is not embeddable — the worked instance being the **rolled Hessian** at 37.5 KB of stack for n=64, 63x hand-written, ERT-clean and green under every pre-ADR-0035 test (#217); `EnableDynamicMemoryAllocation=false` rejects only *unbounded* varsize, so the compiled `-fstack-usage` footprint (`measureErtFootprint`, R17c) is gated too - **not** by an invented byte ceiling but against the stack a **hand-written** derivative of the same maths needs (`generated <= K x hand-written` with **K per case** - 1.5x at parity, 4x where a bounded constant-factor gap is understood - ADR-0035, TS-S-09). A flatness rule was measured and rejected: even optimal hand-written `diag(exp(x))` grows. That is what makes "codegens" mean "embeddable" *(done, #80a-2)*. Rolled scalar-cost gradient/Hessian are green as of R20 Path A (#89, `SRolledErtCodegenTest` / TS-S-06); the still-open cells are the **unrolled** form and **reverse-mode** (deferred to R21), plus R20's remaining CI switch — **(b) done (#92: `SCodegenTest` + `derivShowcaseC` lib builds now `ecoder`)**, **(c) spec'd born-ERT** (ADR-0014 amended, #80 R20c — the campaign oracle's build targets `ecoder`; the oracle implementation is R15/maintainer-authored), (d) option-C printer suppression. Note the **unrolled** cell is open at the *acceptance* stage, not the footprint one: it does **not** ERT-codegen at all — nor under plain `lib` — so its once-quoted O(n²) stack figure cannot be reproduced (#216). License-gated — and **not verifiable on hosted CI**: Embedded Coder is unlicensed there, so every ERT cell is *Filtered* inside a green job. **Verified by a LOCAL run only** (§3.2 "CI cannot verify codegen"). |
...
| TS-U-08 | `UCoreErrorHygieneTest` (gated, `tests/unit`): a successful transform AND a malformed one (injected failure) must each leave `path()`, the open-file set, and `who('global')` unchanged; **and a generation that follows a failed one must still succeed and be numerically correct** — re-entry, checked against the analytic gradient, since released state is a weaker claim than a usable engine. Sampled at BOTH failure modes: `generationSucceedsAfterAFailedGeneration` injects an undefined-variable fixture, which dies in the initial test evaluation (`adigator.m:242`) BEFORE `Tfid`/`Dfid` open, and `generationSucceedsAfterAFailureWithHandlesOpen` injects an unoverloaded operation (`sort`, B40), which passes that evaluation and dies inside the transformation pass (`:775`) with both handles open — the only mode that can reach a closed-print-FID hazard, and the one every other injection in the repo misses. Also pinned in the extended suite by the Monte-Carlo `mcGenNegative` / `oracleHygiene` pair (`MCSmokeTest.{negativeHygieneIsClean,successLeavesNoOpenHandles}`). Pins the B16 `adigator.m` error-path cleanup (ADR-0011); B13 family now pinned. | REQ-C-09, REQ-T-07 |
...
| TS-U-21 | `UGenerationStampTest` (`tests/unit`) — the provenance stamp every generated file carries (issue #200): same version+options+sources give the same id; a changed source, option or tool version moves it; **line endings and trailing whitespace do NOT** (post-#212 the tree is LF, but a checkout or editor can still hand CRLF, and the id must not move for a reason invisible in the file); the id is **independent of absolute path and of closure order** (hashing paths would make it machine-specific, and `mydepfun` does not promise a stable order — either would break the id's only job, telling a maintainer whether a committed derivative still matches its source); an unreadable source degrades rather than throwing, since provenance must never be why a differentiation fails; the timestamp is ISO 8601 UTC. Also pins the header's agreed content and, specifically, that it does **not** route users to the upstream maintainer's email or the sourceforge forums — the defect #200 opened with. License-free. | REQ-T-06 (partial: stamp determinism, **not** generated-file byte-reproducibility — since #200 generated wrappers and derivative files carry a wall-clock line, so they reproduce only modulo timestamps, which is what TS-I-03 compares. The embedded data functions keep the stricter #21 byte-identity) |
| TS-U-22 | `UGateArmedNoticeTest` (`tests/unit`) — the unarmed-pre-push-gate notice (#82, #245): silent when `core.hooksPath` is armed and on CI (a notice on every runner lint run is noise that teaches people to skim `ci_lint` output), printed with its one-line fix when unarmed, and **not** fooled by a lookalike path such as `.githooks-disabled` — a substring match would read that as armed and go quiet about a hook that never runs, which is the exact failure the notice exists to make visible. Advisory output, but a documented failure direction, which `REVIEW_CONTEXT.md` §Red flags requires be asserted rather than described. License-free. | REQ-C-11 (the local gate reproduces CI — an unarmed gate reproduces nothing) |
...
| TS-I-04 | *(planned)* `ISecondDerivTest` — gradient+Hessian through `adigatorGenHesFile` for the `logsumexp` and `gapfun` fixtures, checked against **analytic** Hessians (an FD-independent enhancement). Hessian correctness is already guarded without it: TS-I-01 (shape + exported structure + FD), TS-I-12 (`HessianCSC`), TS-I-13/14/15, and the CasADi oracle TS-S-05. | REQ-T-01, REQ-C-04 |
...
| TS-I-12 | `IOutputModesTest` — the output-form modes (R5 + R25/#84, ADR-0022; respelled to CSC by #192/ADR-0030): `der_output='csc'` returns the value vector reconstructing (via `adigatorCSCToSparse` over `output.<Role>CSC`) to the dense Jacobian/Hessian for scalar **and** `m>1` vector functions and the scalar-of-matrix remap, with no dense projection or `sparse()`; `adigatorGenJtVFile` computes `Jᵀv`. Pins **decision b** (`derOutputSelectsTopOrderOnly` — `der_output='csc'` flips only the top-order Hessian; the companion gradient stays matrix; the removed `jac_output`/`'nonzeros'` spellings error) and the forward-gradient `[Grd, Fun]` name convention. | REQ-T-11, REQ-T-02 |
...
| TS-I-20 | `ILoopboundTest` — the `LOOPBOUND` runtime-loop-bound option (R3, #6 Tier 1): generate at `Nmax`, evaluate for `n <= Nmax` without regeneration with exact structural zeros beyond `n`, the padding-benign contract and the `assert(n <= Nmax)` guard. Since B35 also pins the guard's **position** (`guardPrecedesEveryBoundDependentSize`): it must be the first body statement, ahead of every reference to the bound — the license-free **proxy** for the ERT acceptance TS-S-06 proves but hosted CI cannot run (§3.2, ADR-0034). It is a proxy, not a verification: REQ-T-10's criterion is `ecoder` codegen exit-success, which a text check cannot satisfy — TS-S-06 remains what verifies REQ-T-10 here. | REQ-T-02 (loopbound) |
...
| TS-I-29 | `IRolledOvermapWidthTest` — **B37** (ANALYSIS §1.3k, issue #217, [ADR-0036](decisions/ADR-0036-overmap-directed-pruning-rolled-printing-run.md)): the printing run of a rolled loop must not carry a second-derivative pattern wider than the loop overmap it is about to be squeezed into. The **license-free** half of TS-S-09's #217 catch: the n² stack temporary was gathered *through* an n² static second-derivative index table, so `Gator2Data`'s width tracks the defect exactly and needs no toolchain — asserted both as a magnitude (`<= 4n`) and as a **growth law** (doubling n must about double it, not quadruple it), at n = 8 and 16 where the pre-fix tables were 64 and 256. `Gator1Data` is deliberately *not* asserted on: its per-iteration mask table is `[nzover x niters]`, quadratic by construction in a rolled loop, in int8 ROM rather than stack doubles. Values are pinned too — the diagonal case against its closed form and finite differences, and a **genuinely dense** rolled Hessian (`y = y + sum(x)*exp(x(k))`) against finite differences, which is the over-pruning guard: an over-eager prune would silently zero real off-diagonal terms (principle 1). That dense case additionally asserts its own width comes out **quadratic**, so it proves the prune ran over a wide pattern and kept it rather than being a second diagonal case in disguise. Plus a **static** guard (`pruneGateStaysInStepWithTheRemapGate`): the prune is safe only because `cadaOverMapTargetNz` gates on the same `OVERMAP.FOR(id,1)` + `NAMELOCS(id,3)` pair `cadaOverMap` uses to decide whether it remaps at all, so that mirror is asserted (comment-stripped, verified to discriminate) rather than left to vigilance — ADR-0036's "keep in step" obligation made mechanical. | REQ-T-01, REQ-T-10 |
| TS-I-30 | `IScatterPrototypeTest` — **R21 acceptance reference** (issue #80, [ADR-0019](decisions/ADR-0019-rolled-embeddable-path-scatter-deferred.md); engine-v2 analysis §3.2, experiment E6): the fixed-size **scatter** form — per-iteration operand width `nz_k`, result scattered through a position map — must compute **exactly** what today's rolled emission computes. One padded artifact at `Nmax=16`, five runtime trip counts including `n=1,2` (where small-`n` structural collapse would most plausibly diverge), compared at `AbsTol 0`; the reference is independently pinned against the closed form and finite differences so the class cannot certify a shared error, the padded tail is required to be a structural zero (HZ-3), and the `nz_k = 1` premise is read off the emitted mask so the equivalence cannot pass vacuously. **License-free** — E6's footprint half (ROM 4400→160 B, stack 352→96 B) needs Coder and cannot run on hosted CI (§3.2); the value half is what principle 1 turns on and needs no toolchain. **Two anchors:** the allocation shape (`nz_k = 1`, identity map) and a pair shape (`y = sum_k x(k)*x(k-1)`, `nz_k = 2`, map `[c; c+1]` — width > 1 and not the identity, so the gather/compute/scatter machinery is genuinely exercised). **Scope:** neither reaches **HZ-1** (duplicate rows within one column), because `[c; c+1]` has distinct rows — covered instead by the RUNFLAG-1 census, ANALYSIS §2.5: 2130 observations across the repo's own suites plus adversarial shapes, **zero** duplicate columns, with a structural reason (`cadaunion` → `sparse`+`find`). Exists as a test rather than a `bench/` script because ADR-0019's O(n²) stack figure was quoted into durable documents from an unreproducible measurement, did not reproduce, and had to be retracted (#216). | REQ-T-01, REQ-T-02 (padded structural zeros) |
| TS-I-31 | `IVectorizedFenceTest` — **B40 self-healing tripwire** (ANALYSIS §1.3n; issue #6 Tier 2): a contraction OVER the vectorized (`Inf`) dimension (`x'*x`) escapes `mtimes`' own guard, which keys on the *result* dims (`mtimes.m:129`), and dies in a MATLAB builtin at `:162`; and the eleven designed vectorized refusals are bare `error('...')` with **no identifier**, so none is programmatically catchable. Pins only that the failure is **not ours** — deliberately NOT `MATLAB:nonaninf`, a MATLAB-internal id and an artifact of `true(...,Inf)`, the unstable kind #226 refused to pin. Either half of B40 being fixed makes it fire and names what to retire. Asserts the refusal still happens first, so it cannot pass by the engine starting to ACCEPT the reduction — which would be principle 1, not diagnostics. License-free. | REQ-T-07 (clean errors) |
| TS-I-28 | `ISpecializedTripCountTest` — **B36** (ANALYSIS §1.3j, issue #210): a loop range naming a main-function input specializes the generated file to one trip count, and the file must state it. The guard is the **first** body statement and uses `==` (not `loopbound`'s `<=`); a call above the specialized count errors instead of silently returning the analyzed problem's answer; a declared `loopbound` name keeps `<=` and its padded semantics; a loop bound that is a **local**, not an input, earns no guard. The load-bearing case is **3rd order**: from level 2 the source's loop header is already a literal, so the parameter name survives only by recognizing, dropping and re-recording the previous level's guard — exactly one guard per level, with `f`/`dx`/`dxdx`/`dxdxdx` analytic-exact on `sum(x_k^3)`. 2nd order is where that path first runs; 3rd is where it runs TWICE, which is what distinguishes a one-shot recognizer bug from a cumulative one. **Route 1 (B38, #213)**: the same silent-wrong one call deep - `main(x,N)` handing `N` to a subfunction that loops on it - closed by **interprocedural resolution** rather than by loosening the main-function gate (which is what stops a shadowing parameter forging a guard, and whose pin must keep passing). A guard is earned only when **every** call site is in the main function and passes the same *bare identifier*; resolution follows the argument, so `main(x,M)` guards `M` even where the callee spells it `N`. The caller check is load-bearing: an argument names something in the CALLER's scope, so a site inside another subfunction would resolve by name equality - the shadowing forge one hop deeper, firing on code that is correct for every N. Fail-closed and pinned on: a two-hop local, an expression argument, a local, and **disagreeing call sites** (one printed body serves all sites). Survives to 2nd order via `adigatorGenHesFile` (one guard per level, `sum(x^3)`->`3x^2`->`diag(6x)` exact); 3rd order is **impossible for this shape** because re-differentiating a subfunction-calling generated file fails for pre-existing, unrelated reasons - that boundary is itself pinned, so restoring the 3rd-order case is a red test away. 4(a) (B39, #213)**: a second loop over a declared `loopbound` name whose trip count does NOT match the declared maximum (`for a = 1:N` alongside `for b = 1:N-1`) matched no bound, took a literal header inside a runtime-bound file, and was silently wrong for every call below the maximum - the file's own `assert(N <= Nmax)` being satisfied by exactly those calls. Refused for an EXPRESSIVE reason: a `1:N-1` header would be correct under the file's own assert, but matching is by trip-count value and only `1:<name>` headers can be emitted (affine bounds are #6 Tier 2), so generation now **refuses** with `adigator:loopbound:rangemismatch`. Narrowed to the main function and to ranges that NAME a bound - a literal `1:3` and a field `1:p.N` still generate; one level of indirection (`M = N-1`) still escapes, recorded as residual. Pinned both ways: the mismatched shape errors, and two matching loops / a deliberate literal `1:3` / the ordinary single-loop case all still generate with padded semantics intact. | REQ-T-01, REQ-T-02 |
...
| TS-S-04 | `MCSmokeTest` + `tests/montecarlo/mcCampaign` — randomized-function campaign over the generators (affine / quadratic / flat shape-fuzz, and the **nested typed-AST `mcGenExprTree`** — #38 Phase C — which emits closed-form-free vector-Jacobian *and* scalar-Hessian cases) checked by the tolerance-free oracles (cross-mode exact, known-derivative, sparsity-superset, **param-delivery invariance** — `oracleParamDeliveryInvariance`, the R27/#103 input-topology backstop for the B17/B22 class; **FD secondary** — `oracleFiniteDiff`, now including the **FD-Hessian** path that value-checks the closed-form-free scalar-Hessian cases via two tight 1st-order FD checks, G-vs-FD-of-f and H-vs-FD-of-G, #38 Phase C), plus, where MATLAB Coder is available, a **sampled** `oracleCodegenEquivalence` (**born ERT**, hand-rolled `codegen`+compare) that builds the embedded `'i'` wrapper through Embedded Coder (`coder.config('lib','ecoder',true)`, proving strict-target codegen) plus a MEX and asserts compiled-C ≡ MATLAB over `c.x0` + a few perturbations (ADR-0014, issue #64; `MCSmokeTest.codegenEquivalenceIsClean` runs a tiny deterministic set, the full sampled sweep is a release-checklist `mcCampaign` including this oracle). Migrating it to the `matlabtest.coder.TestCase` supported API + `GeneratedCodeCoveragePlugin` generated-C coverage is *(planned, R15 — needs MATLAB Test R2023a+)*. Failures feed delta-debug shrinking and automatic fixture promotion. `MCSmokeTest` runs a fixed-seed, fixed-iteration subset in the extended (per-merge) suite; the unbounded campaign is an opt-in local / release-checklist run, and the codegen-equivalence oracle (sampled, license-gated) runs only in that fuller run. *(Issue #38, ADR-0007, roadmap R9 / R14 / R15; codegen oracle issue #64, ADR-0014.)* | REQ-T-09 (cross-validates REQ-T-01..05 at scale) | base MATLAB (Coder / MATLAB Test oracles skip-clean) |
| TS-S-06 | `SRolledErtCodegenTest` — **ERT regression guard** (#80 Gap B / Path A): the rolled (`unroll=0`) `scostfun` gradient + Hessian must codegen under the strict shared Embedded Coder config (`adigatorCoderConfig('GenCodeOnly',true)`, ADR-0033) at **n=32** — the size where identical index tables get de-duplicated, the regime where the old `S.x.IndexN = S.x.IndexM` static-data self-alias broke ERT ("field added after struct read"). Pins ERT *acceptance* end-to-end (the unit test `UEmbedMfileTest` pins only the emitted form). Coder + Embedded Coder gated; skips clean without the licenses. **Also** (B35, ADR-0034) `loopboundGradientErtCodegenStaticMemory`: a `loopbound` gradient must code-generate with `EnableDynamicMemoryAllocation = false` — the no-heap regime the target actually runs in, and the setting that made the too-late runtime-bound guard visible (with the heap enabled the file generates either way and the assertion proves nothing). | REQ-T-10 | MATLAB Coder + Embedded Coder (skip-clean) |
| TS-S-09 | `SStackScalingTest` — **the embeddability gate** (#80a-2, ADR-0035): a generated derivative's max stack must stay within a **per-case tolerance** (1.5x at parity, 4x where a bounded constant-factor gap is understood) of the hand-written analytical derivative of the same function (`bench/showcase/analytic/`, themselves FD-checked by TS-S-07/`SDerivShowcaseTest`), over a size sweep. Closes REQ-T-10's necessary-but-not-sufficient gap: exit-success says the file builds, this says it does not carry runaway stack overhead versus optimal hand code. (It bounds the generator's overhead; it is not an absolute guarantee against a given device's stack - no target is declared, ADR-0035.) Measured at n=64: vectorized gradient **1.00x** (byte-identical to hand-written), vectorized Hessian 1.05x, **subscripted Hessian 1.03x**, `vvecfun` Jacobian 2.64x - a bounded constant factor, `(112+24n)/(112+8n)` -> 3.0, not an asymptotic gap. The subscripted Hessian was this gate's first catch and shipped as a self-healing `KnownIssue` pin on issue #217 at **28.6x (n=32) / 63.4x (n=64, 37.5 KB of stack)** while ERT-codegenning cleanly and passing TS-S-06 - exactly the hollow milestone. [ADR-0036](decisions/ADR-0036-overmap-directed-pruning-rolled-printing-run.md) closed it (37,552 -> 608 B at n=64, i.e. `96+8n`, the same series as its vectorized twin), so it is now an ordinary **parity** assertion at 1.5x, no longer tagged. Coder + Embedded Coder + standalone `gcc`/`size` gated; ~170 s. **Local only** (§3.2); `tests/ci_ert.m` is the release-attestation entry point. Its license-free companion is TS-I-29. | REQ-T-10 | MATLAB Coder + Embedded Coder + gcc/size (skip-clean) |
| TS-S-10 | `SGenericBoundFoldingTest` — **R6 Tier-2 / Route B premise** (issue #6; engine-v2 analysis §4, experiment E2): a generic kernel carrying the three constructs Route B would emit (`zeros(N,1)`, an affine `uint32(reshape(1:1:N,N,1))` index generator, `for k = 1:N`) compiled with `coder.Constant(N)` under the strict no-heap profile must carry **no `static const` table and no `.rdata` at all** — the generator is eliminated into the loop induction variable, not folded into a table — while the same source with a **runtime** `N` keeps an index-materialization local and strictly more ROM (the non-vacuity contrast). Asserts properties, not byte counts, which are toolchain-specific. **Coder-gated, local-only** (§3.2). *Verifies a design premise, not a shipped requirement:* it guards a claim about the **toolchain** that a Coder upgrade or config change could silently invalidate while Route B rested on it. Deliberately mapped to no REQ — adding one is a maintainer decision (CLAUDE.md §4) and should wait until/unless R6 goes. | *(none — design premise; see note)* | MATLAB Coder + Embedded Coder + standalone gcc/size (skip-clean on either) |
...
| TS-S-08 | `SCscMetadataTest` — pins `bench/cscMetadataSize.m` (#192, ADR-0030 phase-C bench, `bench/SHOWCASE.md` ## CSC pattern metadata): the exported CSC pattern index count is `nnz+ncols+1`, smaller than the removed two-copy surface (coordinate `*Locs` `2·nnz` + sparse `*Structure`), beats coordinates outright when `nnz>ncols+1`, and pins each shape's structural `nnz` in closed form (gradient `n`, arrowhead `3n−2`, diagonal `n`) so an overmap-widened pattern (B23/B27 class) trips here. Base MATLAB, no Coder; runs in the **extended suite (per-merge)** like TS-S-07, not the PR gate. | REQ-T-09 (bench), REQ-T-03 | base MATLAB |

| Requirement | Verified / validated by |
|-------------|-------------------------|
| REQ-T-01 | TS-I-01, TS-S-01, TS-I-13, TS-I-14, TS-I-15, TS-I-26 (zero/linear-objective Hessian, B32), TS-I-27 (@cadastruct fallback naming, B29-B31/B33-B34), TS-I-28 (specialized trip count, B36), TS-I-30 (R21 scatter acceptance reference), TS-I-29 (rolled overmap width, B37); TS-I-04 *(planned)*, TS-I-10 *(planned, R22)*, TS-I-25 |
| REQ-T-02 | TS-I-01, TS-I-05, TS-I-12, TS-I-20, TS-I-30 (padded structural zeros), TS-I-26 (zero-Hessian returned shape), TS-U-19 (loopbound); TS-I-10 *(planned, R22)* |
...
| REQ-T-06 | TS-I-03 (byte-reproducible modulo timestamp lines), TS-U-21 (generation-stamp determinism) |
| REQ-T-07 | TS-U-08, TS-I-16, TS-I-31 (B40 vectorized-fence tripwire) |
| REQ-T-08 | TS-S-01 |
| REQ-T-09 | TS-S-04, TS-S-05, TS-S-07, TS-S-08 |
| REQ-T-10 | TS-S-06 (rolled-form ERT acceptance, #89), TS-S-09 (stack overhead vs hand-written, ADR-0035), TS-I-29 (its license-free companion — rolled overmap width, B37/ADR-0036); TS-S-02 migrated to the ERT `ecoder` target (#92); TS-S-04 (MC oracle) spec'd born-ERT (ADR-0014 amended, R20c; oracle implementation is R15); issue #80 |
