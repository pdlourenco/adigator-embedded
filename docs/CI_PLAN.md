...
| TS-I-06 | `IEmbedSlimTest` — the `slim_embed` driver end-to-end (roadmap R7b, issue #21; ADR-0006): generating a structurally sparse Jacobian in coderload mode with vs. without `slim_embed`, asserting the slimmed derivative code drops the unread `_location` metadata, the pruned data is no larger, the numeric result is unchanged (coder-gated runtime check), and that classic mode is a byte-for-byte no-op. | REQ-T-04 |
