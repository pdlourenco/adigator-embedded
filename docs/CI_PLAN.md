...
| REQ-T-11 | **Generalized derivative output form.** Per ADR-0022 (C-6 output-form facet + C-2), the output form shall be selectable via `der_output ∈ {matrix, nonzeros}` applied per derivative level (`jac_output` a back-compat alias for the first-derivative level); in `nonzeros` form each supporting DerType shall export its constant sparsity pattern once via its `*Locs` companion (`HessianLocs` the `output.JacobianLocs` analog), reconstructing the dense derivative exactly. Every wrapper's outputs shall use the canonical C-6 **names** — including the forward gradient returning `Grd` (not `Jac`). Which `(der_output × DerType × mode)` cells are supported vs **N/A** is a documented matrix (DESIGN + README), N/A by design (e.g. `classic + slim`; `nonzeros` of a dense gradient; `nonzeros` of reverse-grad/`JtV`). *(Planned, roadmap R25 — Hessian-nonzeros first, the R22/#85 prerequisite; issue #84.)* | `nonzeros` + `*Locs` reconstructs the dense derivative to FD tolerance (`HessianLocs` first); the option matrix's supported cells emit the selected form and the N/A cells error/skip as documented; the forward-gradient signature asserts `[Grd, Fun]`. *(Planned, R25.)* |
...
| TS-U-17 | *(planned, R25 — issue #84, ADR-0022)* `UHessianLocsTest` — the generalized `der_output='nonzeros'` output form for the Hessian: `HessianLocs` (the `output.JacobianLocs` analog) + the nonzero vector shall reconstruct the dense Hessian to FD tolerance (the TS-U-03 analog, one order up), and the forward gradient wrapper's signature shall be `[Grd, Fun]` (the confirmed-live `Jac`→`Grd` C-6 name fix folded into R25 phase 1). | REQ-T-11, REQ-T-02 |
...
| TS-S-04 | `MCSmokeTest` + `tests/montecarlo/mcCampaign` — randomized-function campaign over the generators (affine / quadratic / shape-fuzz → expression-tree) checked by the tolerance-free oracles (cross-mode exact, known-derivative, sparsity-superset; FD secondary), plus — where MATLAB Test + Coder are available — a **sampled** `oracleCodegenEquivalence` that compiles the embedded-mode wrapper and asserts compiled-C ≡ MATLAB over the case (`matlabtest.coder.TestCase`; ADR-0014, issue #64), with delta-debug shrinking and automatic fixture promotion. `MCSmokeTest` runs a fixed-seed, fixed-iteration subset in the extended (per-merge) suite; the unbounded campaign is an opt-in local / release-checklist run, and the codegen-equivalence oracle (sampled, license-gated) runs only in that fuller run. *(Issue #38, ADR-0007, roadmap R9 / R14 / R15; codegen oracle issue #64, ADR-0014.)* | REQ-T-09 (cross-validates REQ-T-01..05 at scale) | base MATLAB (Coder / MATLAB Test oracles skip-clean) |
...
|-------------|-------------------------|
| REQ-T-01 | TS-I-01, TS-I-04, TS-S-01; TS-I-10 *(planned, R22)* |
| REQ-T-02 | TS-I-01, TS-I-05; TS-I-10, TS-U-17 *(planned, R22/R25)* |
| REQ-T-03 | TS-I-01 |
| REQ-T-04 | TS-I-02, TS-I-06, TS-I-08, TS-I-09; TS-I-11 *(planned, R24)* |
| REQ-T-05 | TS-S-02 (cross-validated at scale by TS-S-04 codegen-equivalence oracle, ADR-0014) |
| REQ-T-06 | TS-I-03 |
| REQ-T-07 | TS-U-08 |
| REQ-T-08 | TS-S-01 |
| REQ-T-09 | TS-S-04 |
...
| REQ-T-11 | TS-U-17 *(planned, R25 — Hessian-nonzeros/`HessianLocs` + the `Grd` name fix; issue #84)* |
| REQ-C-01 | TS-U-01 |
| REQ-C-02 | TS-U-02 |
| REQ-C-03 | TS-U-03 |
