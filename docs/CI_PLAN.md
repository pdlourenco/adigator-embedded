...
| TS-S-04 | `MCSmokeTest` + `tests/montecarlo/mcCampaign` — randomized-function campaign over the generators (affine / quadratic / shape-fuzz → expression-tree) checked by the tolerance-free oracles (cross-mode exact, known-derivative, sparsity-superset, **param-delivery invariance** — `oracleParamDeliveryInvariance`, the R27/#103 input-topology backstop for the B17/B22 class; FD secondary), plus — *(planned, R15)*, where MATLAB Test + Coder are available — a **sampled** `oracleCodegenEquivalence` that compiles the embedded-mode wrapper and asserts compiled-C ≡ MATLAB over the case (`matlabtest.coder.TestCase`; ADR-0014, issue #64; no such oracle asset exists yet), with delta-debug shrinking and automatic fixture promotion. `MCSmokeTest` runs a fixed-seed, fixed-iteration subset in the extended (per-merge) suite; the unbounded campaign is an opt-in local / release-checklist run, and the codegen-equivalence oracle (sampled, license-gated) runs only in that fuller run. *(Issue #38, ADR-0007, roadmap R9 / R14 / R15; codegen oracle issue #64, ADR-0014.)* | REQ-T-09 (cross-validates REQ-T-01..05 at scale) | base MATLAB (Coder / MATLAB Test oracles skip-clean) |
...
| REQ-T-05 | TS-S-02; cross-validation at scale by the TS-S-04 codegen-equivalence oracle is *(planned, R15, ADR-0014)* |
| REQ-T-06 | TS-I-03 |
