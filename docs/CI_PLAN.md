...
| TS-I-16 | `ISymbolicIndexTest` — **B19 / B20** actionable-error guard (ADR-0024): data-dependent / symbolic indexing (a `while`-counter or aux-valued subscript) raises an **actionable** error naming the construct and the logical-weight-sum rewrite — never a wrong derivative (REVIEW_CONTEXT principle 1); also scopes the B19 `if`-guarded over-approximation residual (#108). | REQ-T-07 |
| TS-I-17 | `IEmbedUnsupportedTest` — the **ADR-0023** embed-mode source-scan gate (B21 `load` / B22-in-embed cells): embed generation rejects cells / `load` / `global` with an actionable `adigator:embed:unsupportedConstruct` error at generation time, while classic mode stays permissive. | REQ-T-04 |
...
|-------------|-------------------------|
| REQ-T-01 | TS-I-01, TS-I-04, TS-S-01, TS-I-13, TS-I-14, TS-I-15; TS-I-10 *(planned, R22)* |
...
| REQ-T-11 | TS-I-12 (Hessian-nonzeros/`HessianLocs` + the `Grd` name fix, R25/#99); phase-2 support-matrix + per-level selection *(planned, R25)* |
| REQ-C-01 | TS-U-01 |
| REQ-C-02 | TS-U-02 |
| REQ-C-03 | TS-U-03 |
