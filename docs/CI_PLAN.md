...
| TS-U-08 | `UCoreErrorHygieneTest` (gated, `tests/unit`): a successful transform AND a malformed one (injected failure) must each leave `path()`, the open-file set, and `who('global')` unchanged; **and a generation that follows a failed one must still succeed and be numerically correct** (`generationSucceedsAfterAFailedGeneration` — re-entry, checked against the analytic gradient, since released state is a weaker claim than a usable engine). Also pinned in the extended suite by the Monte-Carlo `mcGenNegative` / `oracleHygiene` pair (`MCSmokeTest.{negativeHygieneIsClean,successLeavesNoOpenHandles}`). Pins the B16 `adigator.m` error-path cleanup (ADR-0011); B13 family now pinned. | REQ-C-09, REQ-T-07 |
