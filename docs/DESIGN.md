...
| `tests/`, `examples/` | The `matlab.unittest` suite (`tests/{unit,integration,system}`, driven by `tests/ci_local.m`); worked examples (jacobians, hessians, stiff ODEs, optimization); and `tests/legacy/test_unarymath_rules.m`, the legacy finite-difference rule harness the suite was built from. |
