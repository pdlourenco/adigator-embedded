...
| R5 | **Output-form optimizations** (re-scoped per ANALYSIS §2.3(5)): `jac_output='nonzeros'` wrapper mode (nonzero vector with the constant pattern exported once via `output.JacobianLocs`, no per-call dense projection) and `adigatorGenJtVFile` (`J'·v` in one forward+adjoint sweep on the R4 reverse engine, runtime `v`). Dead-code slicing deferred: `_RGrd` files already emit only needed statements; forward-file slicing belongs in the printer (Stage 1) and is folded into the R6 decision. Test `tests/integration/IOutputModesTest.m`. | ANALYSIS §2 | in progress |
