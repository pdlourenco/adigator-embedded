...
% lmodern: scalable Latin Modern (visually ~identical to the default Computer
% Modern) so microtype font expansion works (expansion needs scalable fonts).
\usepackage{lmodern}
% microtype: character protrusion + font expansion micro-adjust glyph widths to
% clear the small prose overflows from long \texttt tokens (function names,
% option strings) without visible loosening. No shell-escape; shifts pagination
% slightly, which is fine (the PDF is a build artifact rebuilt by CI).
\usepackage{microtype}
% emergency stretch absorbs the small prose overflows (long \texttt tokens) by
% stretching interword glue only on lines that would otherwise be overfull -
% cleaner than a global \sloppy. Does not help glue-free lines (table
% signatures, display equations), which are fixed structurally instead.
...
\usepackage{fancyvrb}
\usepackage{listings}
% ADR-0025: samples that reproduce GENERATED output are loaded from committed
% golden fixtures via \lstinputlisting keyed on named comment-marker ranges (not
% line numbers), so they cannot silently drift from what the generator emits.
% Hand-written user code keeps using verbatim. Plain pdflatex, no shell-escape.
% listings reads \lstinputlisting source files byte-wise (independent of the
% document's default UTF-8 input), so non-ASCII bytes in a listed .m file -- e.g.
% the "Lourenço" copyright headers -- need explicit UTF-8 handling here or the
% build dies with "Invalid UTF-8 byte sequence". extendedchars + a literate map
% for the Latin/Portuguese accents render them without a font-encoding change.
\lstset{language=Matlab,basicstyle=\ttfamily\footnotesize,columns=fullflexible,%
  keepspaces=true,breaklines=true,showstringspaces=false,frame=single,captionpos=b,%
  extendedchars=true,inputencoding=utf8,%
  literate={ç}{{\c c}}1 {Ç}{{\c C}}1 {á}{{\'a}}1 {à}{{\`a}}1 {ã}{{\~a}}1 %
    {â}{{\^a}}1 {é}{{\'e}}1 {ê}{{\^e}}1 {í}{{\'i}}1 {ó}{{\'o}}1 {õ}{{\~o}}1 %
    {ô}{{\^o}}1 {ú}{{\'u}}1 {ñ}{{\~n}}1 {É}{{\'E}}1 {Á}{{\'A}}1}
...
{\large A Source Transformation via Operator Overloading \\ Toolbox for the Automatic Differentiation of \\Mathematical Functions in MATLAB}\\
 ...
 \vspace{1.2 cm}
 {\large Embedded fork --- v2.0\\ \vspace{6pt} {\bf Pedro Louren\c{c}o}\, (GMV)}
\end{center}
\end{titlepage}
...
This document describes the base \emph{ADiGator} package. The embedded fork (v2.0; GMV / Pedro Louren\c{c}o, 2025--2026) adds derivative files suitable for embedded code generation (MATLAB Coder), together with several capability and output-form extensions. These are documented in Section \ref{Sec:Embedded} and as additional options in Table \ref{Tab:Options}; the underlying differentiation algorithm is unchanged.
...
\item {\bf Derivative Inputs}: This refers to any input arrays which are to have derivatives with respect to the VOD; these inputs are generated with the \texttt{adigatorCreateDerivInput} command, whose syntax is as follows:\\
...
\end{enumerate}
\end{itemize}
\item {\bf Unknown Auxiliary Inputs}: This refers to any input arrays to the user function which do not contain derivatives with respect to the VOD, but are not a fixed, known, numeric value. That is, they may change numeric values on any given call to the user function file. These inputs are generated with the \texttt{adigatorCreateAuxInput} command, whose syntax is as follows:\\
...
{\bf EMBED\_MODE} & \texttt{[]} & {\bf (new in v2.0)} Unset (default): each entry point resolves its own default --- \texttt{adigatorGenDerFile\_embedded} uses \texttt{'i'} (inline), the other generators use \texttt{'c'} (classic). Set explicitly to override. \\ \cline{2-3}
~ & \texttt{'c'} & {\bf C}lassic. The static index/data tables are stored in a \texttt{.mat} file, loaded at runtime into a global variable. Not suitable for embedded code generation. \\ \cline{2-3}
~ & \texttt{'i'} & {\bf I}nline. The static tables are emitted as source in a companion data function: no \texttt{.mat}, no global/persistent variables, and no runtime load. Fully self-contained for code generation. \\ \hline
{\bf PATH} & \texttt{[]} & {\bf (new in v2.0)} Directory in which to place all generated files. If empty, the calling directory is used (default). \\ \hline
{\bf LOOPBOUND} & \texttt{\{\}} & {\bf (new in v2.0)} All loop bounds are fixed at their generation-time trip counts (default). \\ \cline{2-3}
~ & name(s) & Char/string/cellstr naming input(s) of the differentiated function which act as runtime loop bounds: the file is generated at the maximum trip count (the value passed to \texttt{adigator}) and may then be evaluated for any \texttt{n} up to that maximum. See Section \ref{SubSec:Loopbound}. \\ \hline
{\bf DER\_OUTPUT} & \texttt{'matrix'} & {\bf (new in v2.0)} The canonical output-form option: the wrapper projects the {\em top-order} derivative into a dense/sparse matrix on every call (default). \\ \cline{2-3}
~ & \texttt{'csc'} & The wrapper returns the top-order derivative's structurally-possible-nonzero vector in compressed-sparse-column (CSC) order (values only --- no dense projection or scatter); the constant pattern is exported once as the per-role \texttt{output.*CSC} companion (\texttt{JacobianCSC}, \texttt{GradientCSC}, \texttt{HessianCSC}; fields \texttt{Size}/\texttt{ColPtr}/\texttt{RowIdx}/\texttt{Nnz}/\texttt{IndexBase}), the sole exported sparse-pattern representation in both modes. On a Hessian file this flips only the Hessian output --- the gradient companion \texttt{Grd} stays dense. \\ \hline
{\bf DER\_LEVELS} & \texttt{[]} & {\bf (new in v2.0)} The wrapper returns every derivative level its generator produces (default). \\ \cline{2-3}
~ & vector & A subset of $\{0,1,2\}$ ($0$ = function value, $1$ = first derivative, $2$ = Hessian) selecting which levels the wrapper returns; the top level the generator is named for is always included. \\ \hline
{\bf SLIM\_EMBED} & \texttt{[]} & {\bf (new in v2.0)} Unset (default): \texttt{adigatorGenDerFile\_embedded} defaults this on (embeddable output is wanted); the other generators leave it off. Set 0/1 to override. \\ \cline{2-3}
~ & 0 & No slimming. \\ \cline{2-3}
~ & 1 & In inline mode (\texttt{'i'}), slice from the generated code the statements that feed only output fields the wrapper never reads (the \texttt{\_location}/\texttt{\_size} metadata), so their constant index tables drop from the embedded data. Guarded by a generation-time correctness check; left unsliced on any uncertainty. \\ \hline
\end{longtable}
...
\texttt{output.JacobianCSC} - the CSC sparsity pattern (\texttt{Size}/\texttt{ColPtr}/\\ \texttt{RowIdx}/\texttt{Nnz}/\texttt{IndexBase}) of the Jacobian
\end{tabular}\\ \hline
...
\texttt{output.HessianCSC} - the CSC sparsity pattern of the Hessian (and \texttt{output.GradientCSC} for the companion gradient)
\end{tabular}\\ \hline
{\bf Examples} & \begin{tabular}{l}
\texttt{examples/hessians/logsumexp/}
\end{tabular}
\end{tabular}
\subsection{Solver-Integration Wrappers (Inherited from Upstream)}\label{SubSec:SolverWrappers}
\emph{Note --- deprecated.} The five \texttt{adigatorGenFiles4*} commands documented in the following subsections (for \texttt{fsolve}, \texttt{fminunc}, \texttt{fmincon}, IPOPT, and GPOPS~II) are convenience wrappers carried over from upstream \emph{ADiGator}. They are \textbf{deprecated}: shipped for \textbf{continuity with the upstream repository only}, \textbf{not maintained}, and their functionality \textbf{may be reduced or removed in a future release} without further notice. They emit \textbf{host-only} derivative files (the classic runtime-\texttt{load}/global mechanism) and are \textbf{not at feature parity} with the rest of this fork: they do not support embedded code generation (\texttt{EMBED\_MODE}, MATLAB / Embedded Coder), the \texttt{PATH} option, the \texttt{csc} output form, or reverse mode, and are not maintained or tested to the depth of the core generators. For embeddable derivatives use the core generators instead (Section~\ref{Sec:Embedded}); these wrappers are retained only for upstream continuity, and new code should not be written against them.
...
\subsection{Reverse Mode (Adjoint) Commands}\label{SubSec:ReverseBlackBox}
The routines above accumulate derivatives in \emph{forward} mode, whose cost scales with the number of independent variables. For a scalar-valued cost, or when only the action of the transposed Jacobian on a vector is required, \emph{reverse} (adjoint) mode is far cheaper: it computes the full gradient (or $\bfJ^\top\bfv$) in a fixed number of sweeps --- one forward evaluation plus one adjoint pass --- \underline{independent of the number of variables}. The two black box routines below expose this mode with the same output-structure convention as the forward wrappers (the derivative first, the function value last; see \texttt{docs/DESIGN.md}).

Reverse mode is most useful for \emph{dense} objective gradients --- sums, log-sum-exp, least-squares residual norms --- where the forward Jacobian would be a single full row and the adjoint gradient is one sweep; it also underlies the matrix-free transposed-Jacobian-vector product used by first-order embedded solvers. Both routines operate on the forward derivative file that \texttt{adigator} prints (a static tape of fixed-size statements with constant index maps) and emit a self-contained adjoint file, so the user code must first differentiate cleanly in forward mode with its loops \emph{unrolled} (\texttt{adigatorOptions('unroll',1)}) and its active operations drawn from the supported set: elementary arithmetic, \texttt{mtimes}, \texttt{sum}, \texttt{reshape}, \texttt{transpose}, constant-index gathers/scatters/concatenations, and the common unary math functions (\texttt{sin}, \texttt{cos}, \texttt{exp}, \texttt{log}, \texttt{sqrt}, \texttt{tanh}, \dots). Statements off the path from the variable of differentiation to the output pass through untouched; anything unsupported \emph{on} that path raises a clear generation-time error naming the offending statement.

\subsubsection{adigatorGenRevGradFile}
\underline{When to use}: You wish to compute the gradient of a \emph{scalar} cost with respect to one of its inputs at a cost independent of the number of variables (reverse/adjoint mode).\\\\
\underline{User Function Input Restrictions}: Exactly one input is the variable of differentiation (created via \texttt{adigatorCreateDerivInput}); the file must have a single \emph{scalar} output. Loops must be unrolled (\texttt{adigatorOptions('unroll',1)}); the active operation set is as described above.\\\\
\underline{Generated Reverse-Gradient File Info}: The wrapper file is named with the user's function name and \texttt{\_RGrd} appended. It takes the same (plain numeric) inputs as the original file and returns \texttt{[Grd, Fun]} --- the column gradient first, then the scalar value --- computed by one forward and one reverse sweep.\\\\
\begin{tabular}{l|l}
{\bf Usage} & \begin{tabular}{l}
 \texttt{output = adigatorGenRevGradFile(UserFunName,UserFunInputs)}\end{tabular} \\ \hline
{\bf Input Info} & \begin{tabular}{l}
\texttt{UserFunName} - string name of file to be differentiated (single scalar output)\\
\texttt{UserFunInputs} - cell array of inputs to user's function\\
The cell element corresponding to the variable of differentiation must be created\\ via \texttt{adigatorCreateDerivInput}. All other inputs must be fixed.\\
\texttt{options} - (optional) \texttt{overwrite}, \texttt{path}, \texttt{echo}, \texttt{unroll} are honored.
\end{tabular}\\ \hline
{\bf Output Info} & \begin{tabular}{l}
\texttt{output.FunctionFile} - name of the generated reverse-gradient file\\
\texttt{output.MatFile} / \texttt{output.Path} - generated artifacts (\texttt{MatFile} is\\ empty when the adjoint carries no constant data, as in the example below)\\
Generated file signature: \texttt{[Grd,Fun] = <UserFunName>\_RGrd(inputs)}
\end{tabular}\\ \hline
{\bf Examples} & \begin{tabular}{l}
\texttt{examples/gradients/logsumexp/}
\end{tabular}
\end{tabular}

A concrete sample: the reverse-gradient file \emph{ADiGator} generates for the
weighted log-sum-exp cost $y(\bfx)=\log\sum_i e^{w_i x_i}$ (the
\texttt{examples/gradients/logsumexp/} example). One forward sweep evaluates the
cost; one reverse (adjoint) sweep then accumulates the entire gradient in a
single pass, independent of $\dim\bfx$. Because this fully vectorised cost has a
data-free adjoint, the generated file is self-contained --- no \texttt{.mat}, no
globals. The excerpt below is loaded directly from the committed golden fixture,
...
\lstinputlisting[includerangemarker=false,%
  ...
  {../../tests/fixtures/guide/lse_cost_RGrd.m}

\subsubsection{adigatorGenJtVFile}
\underline{When to use}: You wish to compute the transposed-Jacobian-vector product $\bfJ^\top\bfv$ of a vector function for a \emph{runtime} vector \texttt{v}, without ever forming the Jacobian --- the quantity gradient-based embedded solvers consume directly.\\\\
\underline{User Function Input Restrictions}: As for \texttt{adigatorGenRevGradFile} (one derivative input; loops unrolled; the supported active operation set), except the user function may have a \emph{vector} output. \texttt{v} is a runtime input the size of that output and does not participate in generation.\\\\
\underline{Generated JtV File Info}: The wrapper file is named with the user's function name and \texttt{\_JtV} appended. It takes the same inputs as the original file plus the runtime vector \texttt{v}, and returns \texttt{[Jtv, Fun]}, where \texttt{Jtv = J(x).'*v(:)}, in one forward plus one adjoint sweep (cost independent of \texttt{numel(x)}). A single generated file serves every \texttt{v}.\\\\
\begin{tabular}{l|l}
{\bf Usage} & \begin{tabular}{l}
 \texttt{output = adigatorGenJtVFile(UserFunName,UserFunInputs)}\end{tabular} \\ \hline
{\bf Input Info} & \begin{tabular}{l}
\texttt{UserFunName} - string name of file to be differentiated (vector output allowed)\\
\texttt{UserFunInputs} - cell array of inputs to user's function; the variable of\\ differentiation created via \texttt{adigatorCreateDerivInput}, all others fixed\\
\texttt{options} - (optional) \texttt{overwrite}, \texttt{path}, \texttt{echo}, \texttt{unroll} are forwarded.
\end{tabular}\\ \hline
{\bf Output Info} & \begin{tabular}{l}
\texttt{output.JtVName} - string name of the generated file, ex: \texttt{`myfun\_JtV'}\\
Generated file signature: \texttt{[Jtv,Fun] = <UserFunName>\_JtV(inputs,v)}
\end{tabular}\\ \hline
{\bf Examples} & \begin{tabular}{l}
Shares the reverse engine of \texttt{adigatorGenRevGradFile}\\ (with the output's adjoint seeded by \texttt{v} instead of $1$).
\end{tabular}
\end{tabular}

\section{Embedded and Extended Features (v2.0)}\label{Sec:Embedded}
The features described in this section are specific to the embedded fork of \emph{ADiGator} (v2.0; GMV / Pedro Louren\c{c}o, 2025--2026). They extend the base package with derivative files suitable for embedded code generation (MATLAB Coder), together with several capability and output-form additions. The underlying differentiation algorithm is unchanged; for the architecture and the binding output conventions see \texttt{docs/DESIGN.md}, and for the development log see \texttt{docs/ROADMAP.md} and \texttt{docs/analyses/ANALYSIS.md}.

\subsection{adigatorGenDerFile\_embedded}\label{SubSec:GenDerEmbedded}
\underline{When to use}: You wish to generate an embeddable (code-generation-ready) Jacobian, gradient, or Hessian file, free of the runtime \texttt{load}/global-variable mechanism of the classic generated files.\\\\
\underline{Defaults}: Because this entry point exists to produce embeddable, optimized code, an unset \texttt{EMBED\_MODE} defaults to \texttt{'i'} (inline) and an unset \texttt{SLIM\_EMBED} defaults to on (vs.~classic / off for the other generators). Pass either option explicitly to override.\\\\
\underline{User Function Input Restrictions}: One input must be the variable of differentiation (created via \texttt{adigatorCreateDerivInput}); it may be carried within a scalar struct/cell (see Section \ref{SubSec:StructInput}). The user function may have a single output.\\\\
\underline{Generated File Info}: For \texttt{DerType} \texttt{'jacobian'} the wrapper is named with \texttt{\_Jac} appended; for \texttt{'gradient'}, with \texttt{\_Grd}; for \texttt{'hessian'}, with \texttt{\_Hes} (returning the Hessian, gradient, and function value). The wrapper takes the same inputs as the original file. The number and nature of the additional generated files depends on \texttt{EMBED\_MODE}: classic \texttt{'c'} writes a wrapper, a \texttt{.mat}, and the derivative file; inline \texttt{'i'} writes a single self-contained file with no \texttt{.mat}. Both modes return numerically identical results.\\\\
\begin{tabular}{l|l}
{\bf Usage} & \begin{tabular}{l}
 \texttt{info = adigatorGenDerFile\_embedded(DerType,}\\ \texttt{\ \ \ \ \ \ \ UserFunName,UserFunInputs,options)}\end{tabular} \\ \hline
{\bf Input Info} & \begin{tabular}{l}
\texttt{DerType} - \texttt{'jacobian'}, \texttt{'gradient'}, \texttt{'gradient-reverse'}\\ \ \ \ (reverse-mode adjoint gradient, Section \ref{SubSec:OutputForms}), or \texttt{'hessian'}\\
\texttt{UserFunName} - string name of file to be differentiated\\
\texttt{UserFunInputs} - cell array of inputs to user's function; the variable of\\ differentiation created via \texttt{adigatorCreateDerivInput}, auxiliary inputs via\\ \texttt{adigatorCreateAuxInput}\\
\texttt{options} - (optional) options structure; see \texttt{EMBED\_MODE}, \texttt{PATH},\\ \texttt{DER\_LEVELS}, \texttt{SLIM\_EMBED}, \texttt{DER\_OUTPUT} in Table \ref{Tab:Options}
\end{tabular}\\ \hline
{\bf Examples} & \begin{tabular}{l}
\texttt{examples/jacobians/structinput/} (struct input, inline \texttt{'i'})\\
\texttt{examples/hessians/logsumexp/}\\
\texttt{examples/optimization/pipg/} (Hessian)
\end{tabular}
\end{tabular}\\\\
A minimal example, generating an inline (self-contained) Hessian of \texttt{gapfun} with respect to its second input:
\begin{verbatim}
...
\subsection{N-Dimensional Auxiliary Inputs}\label{SubSec:NDParam}
Although \emph{ADiGator} objects are restricted to two dimensions, the \texttt{adigatorCreateAuxInput} utility accepts an N-dimensional size for {\bf auxiliary} (parameter) inputs, declaring a parameter such as \texttt{B} of size \texttt{m}$\times$\texttt{n}$\times$\texttt{K}. The N-D declaration is folded internally to a two-dimensional array, and trailing-subscript slicing such as \texttt{B(:,:,k)}, or the multi-counter form \texttt{B(:,:,a,k)}, is translated into the corresponding affine column window of the fold, returning an ordinary two-dimensional object. This covers the common ``matrix over time steps (and per actuator)'' case without an N-D object type. See \texttt{examples/jacobians/ndparam/}.

\subsection{Struct Inputs}\label{SubSec:StructInput}
The variable of differentiation and the auxiliary inputs may be carried as fields of a (scalar) struct, to any nesting depth through scalar-struct fields and cell elements (struct arrays are excluded). The convenience and embedded generators locate the variable of differentiation within the struct, seed its derivative along that access path, and forward the remaining fields unchanged; the generated wrapper accepts the same struct shape as the original function. See \texttt{examples/jacobians/structinput/}.

\subsection{Runtime Loop Bounds}\label{SubSec:Loopbound}
The \texttt{LOOPBOUND} option (Table \ref{Tab:Options}) names input(s) of the differentiated function which act as runtime loop bounds. The derivative file is generated at the maximum trip count (the value passed to \texttt{adigator}, used for the analysis), and the matching outer loops are printed with the named input as their bound; loop-exit variables take the union over all iterations. The generated file opens with one \texttt{assert(n <= max)} per declared bound, before any other statement. That placement is load-bearing rather than cosmetic: it is what tells MATLAB Coder the upper bound of \emph{every} size that depends on the parameter --- including your own \texttt{zeros(n,1)}-style allocations --- so the file compiles with static memory allocation (no heap), as an embedded target requires. The generated file may then be evaluated for any $1 \le \texttt{n} \le \texttt{max}$ without regeneration. Results agree with the true \texttt{n}-sized program when the post-loop code is \emph{padding-benign} (sums, dot products, scatter/gather over the loop-written entries); operations that read a fixed-size buffer's padded tail (\texttt{length}, \texttt{end}, \texttt{mean}, \texttt{max}) see the maximum size. See \texttt{examples/jacobians/loopbound/}.

\subsection{Alternative Output Forms and Reverse Mode}\label{SubSec:OutputForms}
...
\begin{itemize}
\item {\bf CSC value-vector output (\texttt{DER\_OUTPUT}).} With \texttt{DER\_OUTPUT = 'csc'} (Table \ref{Tab:Options}) a generator returns its {\em top-order} derivative's structurally-possible-nonzero vector in compressed-sparse-column order (values only), exporting the constant pattern once as the per-role \texttt{output.*CSC} companion, with no per-call dense allocation or scatter. This covers the {\bf Jacobian} (\texttt{output.JacobianCSC}, via \texttt{adigatorGenJacFile}) and the {\bf Hessian} (\texttt{output.HessianCSC}, via \texttt{adigatorGenHesFile}); on a Hessian file it flips only the Hessian output, the gradient companion staying dense. An embedded solver consumes the value vector with the constant \texttt{ColPtr}/\texttt{RowIdx} directly; the host helpers \texttt{adigatorCSCToLocs}/\texttt{adigatorCSCToSparse} rebuild coordinate/sparse forms when needed.
\item {\bf Matrix-free reverse mode.} The transposed-Jacobian-vector product ($\bfJ^\top\bfv$) and the scalar-cost adjoint gradient are provided as the black box routines \texttt{adigatorGenJtVFile} and \texttt{adigatorGenRevGradFile}, documented in Section \ref{SubSec:ReverseBlackBox}. Within the embedded generator, the same adjoint gradient is produced as an {\bf embeddable} file by passing \texttt{DerType = 'gradient-reverse'} to \texttt{adigatorGenDerFile\_embedded}, which routes the adjoint through the classic/inline pipeline exactly like the forward modes.
\end{itemize}

\subsection{Additional Overloaded Operators}\label{SubSec:Operators}
This fork adds the following overloads:
\begin{itemize}
\item \texttt{norm} --- vector $p$-norms ($p = 2,\,1,\,\infty,\,-\infty$, general $p$) and the Frobenius norm (\texttt{'fro'}) are rewritten to elementary differentiable operations. The induced/spectral matrix norms (which would require a singular-value decomposition) raise a clear error rather than returning an incorrect derivative.
\item \texttt{isnan}, \texttt{isinf}, \texttt{isfinite} --- derivative-free predicates evaluated on the value at run time, usable in conditionals and masks.
\end{itemize}

\subsection{Which Embed Mode and Derivative Form Should I Pick?}\label{SubSec:WhichMode}
The two embed modes return identical numbers, so the choice is about the generated artifact. In short: use inline \texttt{'i'} (the default for \texttt{adigatorGenDerFile\_embedded}) for embedded code generation --- it is self-contained (no \texttt{.mat}, no \texttt{load}, no globals) and code-generates under MATLAB Coder / Embedded Coder; use classic \texttt{'c'} for interactive/host use where the runtime \texttt{load} is convenient. For the derivative {\em form}, prefer \texttt{DER\_OUTPUT = 'csc'} when a downstream solver assembles (or never forms) the matrix itself, and the matrix-free products (Section \ref{SubSec:OutputForms}) when only $\bfJ^\top\bfv$ or an adjoint gradient is needed.

The structural difference between the modes is concrete: classic \texttt{'c'} emits three files (a wrapper, a \texttt{.mat} of static data, and the derivative file) that read the \texttt{.mat} through a runtime \texttt{load} and so run only on a host, whereas inline \texttt{'i'} emits a single self-contained file that code-generates to embedded C. Because the classic form cannot code-generate, the on-target cost is a property of the inline artifact, and it is small. Table~\ref{Tab:BenchCompare} reports the compiled \textbf{ROM footprint} (the Embedded-Coder object's \texttt{.text}+\texttt{.rdata}, read with \texttt{size}), the \textsc{mex} runtime, and an interpreted numerical finite-difference cost of \emph{ADiGator}-generated inline derivatives --- forward and reverse gradient, Hessian, and Jacobian --- against hand-written analytic references for the same functions, at a representative small size ($n=8$). At this size the forward- and reverse-mode gradient footprints \emph{converge} and sit modestly above the analytical floor, with zero static RAM: the embeddable forms carry essentially no static data, so automatic differentiation costs little in the embedded artifact. (The compiled ROM is the reliable measurement; the generated-\emph{source}-byte count is a boilerplate-dominated proxy --- dominated by comments and \texttt{initialize}/\texttt{terminate} scaffolding --- and is not a footprint.) Numerical finite differences (\texttt{FD}) appear as their own compiled row --- the same central-difference derivative, written Coder-compatibly and code-generated through Embedded Coder like every other method. Because FD evaluates the cost $n$ times, its compiled ROM is a \emph{multiple} of the hand-derivative's ($4.8\times$ for the gradient, $9.0\times$ for the Hessian at $n=8$); it is the only \emph{inexact} method (central-difference truncation), and its evaluation cost grows $O(n^2)$ against automatic differentiation's $O(n)$, so its disadvantage \emph{grows with problem size}. It is the method one reaches for by default when unwilling to hand-derive, and the table makes the on-target cost of that convenience concrete. The ROM footprint is a deterministic build measurement; the \texttt{MEX/ana} runtime ratios are machine-dependent and noisy --- the compiled evaluations take microseconds, so the ratio of two tiny timings is dominated by call overhead --- and should be read for their \emph{scaling} (see the figure in \texttt{bench/SHOWCASE.md}), not as precise figures. The measurement environment (machine, processor, MATLAB / Coder / compiler versions) is stamped in the table source. A broader comparison of the \emph{generated-code complexity} across every axis (embed mode $\times$ slim $\times$ rolled/unrolled $\times$ forward/reverse) is maintained in \texttt{bench/SHOWCASE.md}.

\begin{table}[htbp]
\centering
\input{bench_compare.tex}
\caption{Compiled (inline embed) derivatives of the four methods --- AD forward, AD reverse, hand-written \emph{analytical}, and numerical finite differences (\emph{FD}) --- at $n=8$, each ERT-compiled to a real footprint: compiled \textbf{ROM} (Embedded-Coder \texttt{.text}+\texttt{.rdata}) and its ratio to the analytic form, plus the \texttt{MEX/ana} compiled-runtime ratio. FD is heavier (it evaluates the cost $n$ times) and the only inexact method. The \texttt{ROM} columns are deterministic build measurements; the \texttt{MEX/ana} ratios are machine-dependent and noisy --- read them for scaling (numerical FD grows $O(n^2)$, AD $O(n)$; see the figure), not as precise figures. Rolled \texttt{vfun} has no analytic counterpart, hence the dashes. Regenerated by \texttt{bench/derivShowcaseC.m}.}
\label{Tab:BenchCompare}
\end{table}

The table above is the \emph{compiled}, code-generation-time comparison. A lot of use --- prototyping, or a design-time simulation loop --- runs derivatives \emph{interpreted}, never compiling; there the ranking shifts (the embed-mode data-load cost and \texttt{slim}'s dead-code trimming matter interpreted in ways the C compiler flattens away). Table~\ref{Tab:BenchInterp} reports the un-gated interpreted view of the same four methods: the generated-code \textbf{complexity} (code lines, deterministic), the interpreted derivative-evaluation time as a ratio to the analytical reference, and the accuracy (\texttt{max err}). The accuracy column is where finite differences stand apart: automatic differentiation and the hand derivative are machine-eps exact ($0$), while FD carries a central-difference truncation error ($\sim\!10^{-10}$ for a gradient, $\sim\!10^{-8}$ for a Hessian). The complexity columns are deterministic; the runtime ratios are machine-dependent (measured on the environment stamped in the table source) and should be read as trends.

\begin{table}[htbp]
\centering
\input{bench_interp.tex}
\caption{Interpreted (no Coder) view of the four methods at $n=6$: generated-code \textbf{complexity} (non-comment \texttt{code lines}), the interpreted derivative-evaluation time as a ratio to the analytical reference (\texttt{interp/ana}), and the derivative error vs.\ that reference (\texttt{max err}). AD and analytical are machine-eps exact; \emph{FD} is the only inexact method (central-difference truncation). For AD/analytical, \texttt{code lines} is the full implementation; for \emph{FD} it is the per-anchor wrapper only (which reuses a shared \texttt{fdDeriv} kernel, not counted), i.e.\ the marginal per-function authoring cost. Complexity is deterministic; the runtime ratios are machine-dependent --- read them as trends, not precise figures. Regenerated by \texttt{bench/derivShowcase.m}.}
\label{Tab:BenchInterp}
\end{table}
...
\subsection{Numeric Outputs (Derivative-File Generators)}\label{SubSec:NumericOutput}
\emph{Note.} The derivative-file generators --- \texttt{adigatorGenJacFile}, \texttt{adigatorGenHesFile}, and \texttt{adigatorGenDerFile\_embedded} --- require the differentiated function to return a \textbf{single numeric array} output. A function that returns a \texttt{struct} (or cell) is \textbf{not supported} by these generators and currently raises an internal error, because a Jacobian/Hessian of a struct-valued output is not a single matrix. Struct/cell \emph{inputs} that carry the variable of differentiation are fully supported (Section~\ref{SubSec:StructInput}); it is only struct \emph{outputs} through these generators that are unsupported. The lower-level \texttt{adigator} command itself \emph{can} differentiate a struct-returning function --- its output is then a struct of derivative structs --- but the matrix-output wrappers cannot assemble their result from it.
...
\emph{Embedded modes.} The allowances above (\texttt{global}/\texttt{load} treated as known auxiliary data) apply to the classic \texttt{'c'} mode. When generating an {\bf embeddable} file --- \texttt{adigatorGenDerFile\_embedded}, or any \texttt{EMBED\_MODE} other than \texttt{'c'} --- a \texttt{global} variable, the \texttt{load} command, or a cell array in the differentiated function makes the generated file {\bf not self-contained}: embedded modes are no more restrictive than classic, so these are emitted verbatim (exactly as in classic mode) and generation proceeds, but a \texttt{adigator:embed:unsupportedConstruct} {\bf warning} names the offending line and notes the file may not code-generate under MATLAB Coder until the construct is removed. To make the file embeddable, pass such data as struct or numeric auxiliary inputs instead, pre-loading any data outside the function. Constructs that classic itself rejects (a bare \texttt{load(.)} with no output, unsupported cell patterns) still raise the usual error.
...
\emph{Note.} Like the other \texttt{adigatorGenFiles4*} wrappers (Section~\ref{SubSec:SolverWrappers}), \texttt{adigatorGenFiles4gpops2} is inherited from upstream \emph{ADiGator} and is \textbf{deprecated} --- unmaintained, and \textbf{not at feature parity} with this fork's embedded pipeline (host-only; no \texttt{EMBED\_MODE}/code generation/\texttt{PATH}/\texttt{csc}/reverse mode). It is retained for upstream continuity only and may lose functionality in a future release.\\
...
When the index is genuinely {\em data-dependent} --- computed from runtime values rather than resolvable to a fixed pattern (for example a subscript derived from a \texttt{while}-loop counter or from the numeric value of an input) --- the operation cannot be handled by static forward AD, and the tool raises an {\em actionable} error that names the offending construct and points to a fixed-subscript rewrite --- a constant logical mask reused for both the reference and the assignment, or the equivalent logical-weighted sum, in the spirit of Section \ref{SubSec:Logical} --- as the supported alternative, rather than silently producing a wrong derivative.
...
This example integrates a moving-mesh discretisation of Burgers' equation (a benchmark from the MATLAB \texttt{ode15s} documentation) using a state-dependent mass matrix. The integrated state $\bfy = [\bfa^T,\bfx^T]^T$ stacks the nodal values $\bfa$ and the moving mesh coordinates $\bfx$. The code in \texttt{examples/stiffodes/burgers/} differentiates the loop-free form of the right-hand side, \texttt{burgersfun\_noloop}, with \texttt{adigatorGenJacFile} --- the loops are removed because the flattened form transforms more efficiently --- and supplies the resulting Jacobian and its sparsity pattern to \texttt{ode15s}, alongside the analytic mass matrix and its sparsity pattern. As with the other stiff-ODE examples, a wrapper adapts the generated file to the input/output scheme \texttt{ode15s} expects. The user may change the problem dimension \texttt{n} and the time span \texttt{tspan}.
...
The code for this example may be found in \texttt{examples/optimization/fminconEx/}. \emph{Note: this example demonstrates a deprecated wrapper (Section~\ref{SubSec:SolverWrappers}); it is retained for upstream continuity and is not embeddable.}
...
\subsection{Extended-Feature and Embedded Examples (v2.0)}
The following examples exercise the fork-specific capabilities of Section \ref{Sec:Embedded}. Each is self-checking --- it compares the generated derivative against a closed form and/or central finite differences and prints a one-line report.

\subsubsection{Reverse-Mode Gradient of Log-Sum-Exp}
The code in \texttt{examples/gradients/logsumexp/} takes the gradient of the scalar cost $\log\sum_i e^{w_i x_i}$ with respect to \texttt{x} using the reverse-mode routine \texttt{adigatorGenRevGradFile} (Section \ref{SubSec:ReverseBlackBox}). The generated \texttt{lse\_cost\_RGrd} returns the full $n$-vector gradient and the value from a single forward and reverse sweep, independent of $n$:
\begin{verbatim}
...
\end{verbatim}
The driver checks the gradient against the closed form $w_i e^{w_i x_i}/\sum_j e^{w_j x_j}$ and against central differences. The differentiated cost function \texttt{lse\_cost.m} is:
\lstinputlisting{../../examples/gradients/logsumexp/lse_cost.m}

\subsubsection{Runtime Loop Bounds}
The code in \texttt{examples/jacobians/loopbound/} demonstrates the \texttt{LOOPBOUND} option (Section \ref{SubSec:Loopbound}): a reduction over \texttt{N} actuators is differentiated once at a maximum trip count \texttt{Nmax}, and the single generated file is then evaluated for several $\texttt{n} \le \texttt{Nmax}$ without regeneration, with exact structural zeros beyond \texttt{n}. This is the ``reconfigurable number of actuators'' use case; the driver confirms the gradient matches the directly computed \texttt{n}-sized problem and that the padded tail stays zero. The differentiated function \texttt{lb\_alloc.m} is:
\lstinputlisting{../../examples/jacobians/loopbound/lb_alloc.m}

\subsubsection{N-Dimensional Parameters}
The code in \texttt{examples/jacobians/ndparam/} declares a parameter \texttt{B} of size $m\times n\times K$ with \texttt{adigatorCreateAuxInput([m n K])} and slices it inside the differentiated function with the natural \texttt{B(:,:,k)} syntax (Section \ref{SubSec:NDParam}). The generated file accepts \texttt{B} either as the 3-D array or as its 2-D column-major fold; the driver checks the Jacobian against the analytic result and finite differences. The differentiated function \texttt{tvmap.m} is:
\lstinputlisting{../../examples/jacobians/ndparam/tvmap.m}

\subsubsection{Struct Inputs}
The code in \texttt{examples/jacobians/structinput/} carries the variable of differentiation and the auxiliary data as fields of a scalar struct (Section \ref{SubSec:StructInput}), exercising four cases: a flat-struct Hessian, a nested-struct Hessian (the derivative field one level deeper), a vector-function Jacobian, and the same flat-struct Hessian through the embedded inline (\texttt{'i'}) pipeline. Each case is checked against the closed-form gradient and Hessian of $\tfrac12\bfx^\top \mathbf{Q}\bfx + \bfc^\top\bfx$. The flat-struct objective \texttt{structobj.m} is:
\lstinputlisting{../../examples/jacobians/structinput/structobj.m}

\subsubsection{Allocation over Time (Free Dimensions)}
The code in \texttt{examples/optimization/vectorized/allocation/} differentiates the per-actuator, per-time-step terms of an optimal-allocation problem \emph{once} in vectorized mode over the product index $i = (k-1)N+a$, leaving both the actuator count \texttt{N} and the horizon \texttt{K} free at runtime. A single generated derivative file then serves every $(N,K)$; the reductions (objective sum, per-time-step moments) are assembled with plain linear algebra by \texttt{alloc\_assemble.m}. See the folder's \texttt{README.md} for the full pattern catalog (product fold, assembly wrappers, folded-2-D parameters, cell arrays).

\subsubsection{Embedded Hessian: PIPG Gap Function}
The code in \texttt{examples/optimization/pipg/} generates an embeddable Hessian of a small cone/gap function, $f = \bfw^\top\,\texttt{conefun}(\bfz) + \bfz^\top\,\texttt{setfun}(\bfz)$, with \texttt{adigatorGenDerFile\_embedded} (Section \ref{SubSec:GenDerEmbedded}), producing the wrapper triple \texttt{[H,G,f] = gapfun\_Hes(w,z)}.
...
This code is contained within \texttt{examples/optimization/fminuncEx/}. The code uses \texttt{adigatorGenFiles4Fminunc} together with \texttt{fminunc} to minimize the brown function. \emph{Note: this example demonstrates a deprecated wrapper (Section~\ref{SubSec:SolverWrappers}); it is retained for upstream continuity and is not embeddable.}
\subsubsection{Banana System of Non-Linear Equations}
This code is contained within \texttt{examples/optimization/fsolveEx/}. The code uses the \texttt{adigatorGenFiles4Fsolve} together with \texttt{fsolve} to solve the banana system of equations. \emph{Note: this example demonstrates a deprecated wrapper (Section~\ref{SubSec:SolverWrappers}); it is retained for upstream continuity and is not embeddable.}
\subsubsection{Minimization Ginzburg-Landau (2-dimensional) Superconductivity Problem}
This code is contained within \texttt{examples/optimization/ipoptEx/}. The code uses \texttt{adigatorGenFiles4Ipopt} together with \texttt{ipopt} in order to minimize the MINPACK-2 GL2 minimization problem. \emph{Note: this example demonstrates a deprecated wrapper (Section~\ref{SubSec:SolverWrappers}); it is retained for upstream continuity and is not embeddable.}
