...
The structural difference between the modes is concrete: classic \texttt{'c'} emits three files (a wrapper, a \texttt{.mat} of static data, and the derivative file) that read the \texttt{.mat} through a runtime \texttt{load} and so run only on a host, whereas inline \texttt{'i'} emits a single self-contained file that code-generates to embedded C. Because the classic form cannot code-generate, the on-target cost is a property of the inline artifact, and it is small. Table~\ref{Tab:BenchCompare} reports the compiled-C size and \textsc{mex} runtime of \emph{ADiGator}-generated inline derivatives --- forward and reverse gradient, Hessian, and Jacobian --- against hand-written analytic references for the same functions, at a representative small size ($n=6$). The generated C is within about ten percent of hand-coded size, and the reverse-mode gradient essentially matches it, so automatic differentiation costs little in the embedded artifact. The compiled-C size is a deterministic build measurement; the \texttt{MEX/ana} runtime ratios are machine-dependent and noisy --- the compiled evaluations take microseconds, so the ratio of two tiny timings is dominated by call overhead --- and should be read as ``comparable to hand-coded'', not as precise figures. The measurement environment (machine, processor, MATLAB / Coder / compiler versions) is stamped in the table source. A broader comparison of the \emph{generated-code complexity} across every axis (embed mode $\times$ slim $\times$ rolled/unrolled $\times$ forward/reverse) is maintained in \texttt{bench/SHOWCASE.md}.

\begin{table}[htbp]
\centering
\input{bench_compare.tex}
\caption{Compiled (inline embed) \emph{ADiGator} derivatives vs.\ hand-written analytic references: generated-C size and \textsc{mex} runtime at $n=6$. The \texttt{C\,(kB)} and \texttt{C\,/\,ana} columns are deterministic build measurements; the \texttt{MEX\,/\,ana} runtime ratios are machine-dependent and noisy (see the table-source note and the surrounding text). Rolled \texttt{vfun} has no analytic counterpart, hence the dashes. Regenerated by \texttt{bench/derivShowcaseC.m}.}
