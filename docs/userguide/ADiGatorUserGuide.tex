...
{\large A Source Transformation via Operator Overloading \\ Toolbox for the Automatic Differentiation of \\Mathematical Functions in MATLAB}\\
 ...
 \vspace{1.2 cm}
 {\large Embedded fork --- post-v1.5 additions\\ \vspace{6pt} {\bf Pedro Louren\c{c}o}\, (GMV)}
\end{center}
\end{titlepage}
...
{\bf EMBED\_MODE} & \texttt{[]} & {\bf (v1.5 fork)} Unset (default): each entry point resolves its own default --- \texttt{adigatorGenDerFile\_embedded} uses \texttt{'i'} (inline), the other generators use \texttt{'c'} (classic). Set explicitly to override. \\ \cline{2-3}
...
~ & \texttt{'l'} & Coder{\bf l}oad {\bf (deprecated)}. The \texttt{.mat} is read through \texttt{coder.load} and wrapped in \texttt{coder.const}: loaded at runtime when interpreted, at compile-time into a persistent constant when compiled with MATLAB Coder. No global variables. \emph{Deprecated} (ADR-0021): \texttt{'l'} does not code-generate under Embedded Coder (the \texttt{coder.load} mechanism), and its compiled footprint converges with \texttt{'i'}, so prefer inline \texttt{'i'}. Retained for now; a one-time deprecation warning is planned. \\ \cline{2-3}
...
{\bf PATH} & \texttt{[]} & {\bf (v1.5 fork)} Directory in which to place all generated files. If empty, the calling directory is used (default). \\ \hline
{\bf LOOPBOUND} & \texttt{\{\}} & {\bf (v1.5 fork)} All loop bounds are fixed at their generation-time trip counts (default). \\ \cline{2-3}
...
{\bf DER\_OUTPUT} & \texttt{'matrix'} & {\bf (v1.5 fork)} The canonical output-form option: the wrapper projects the {\em top-order} derivative into a dense/sparse matrix on every call (default). \\ \cline{2-3}
~ & \texttt{'nonzeros'} & The wrapper returns the top-order derivative's nonzero vector in the fixed pattern order, exporting the pattern once via the matching \texttt{output.*Locs}/\texttt{output.*Structure} companion (\texttt{JacobianLocs} for the Jacobian, \texttt{HessianLocs} for the Hessian); no per-call allocation or scatter. On a Hessian file this flips only the Hessian output --- the gradient companion \texttt{Grd} stays dense. \\ \hline
{\bf JAC\_OUTPUT} & \texttt{'matrix'} & {\bf (v1.5 fork)} Legacy first-derivative-level-only alias of \texttt{DER\_OUTPUT} (\texttt{adigatorGenJacFile}): the Jacobian/gradient is projected to a matrix (default). Prefer \texttt{DER\_OUTPUT}. \\ \cline{2-3}
~ & \texttt{'nonzeros'} & As above, but returns the Jacobian/gradient nonzero vector + \texttt{output.JacobianLocs}; never affects a Hessian. \\ \hline
{\bf DER\_LEVELS} & \texttt{[]} & {\bf (v1.5 fork)} The wrapper returns every derivative level its generator produces (default). \\ \cline{2-3}
...
{\bf SLIM\_EMBED} & \texttt{[]} & {\bf (v1.5 fork)} Unset (default): \texttt{adigatorGenDerFile\_embedded} defaults this on (embeddable output is wanted); the other generators leave it off. Set 0/1 to override. \\ \cline{2-3}
~ & 0 & No slimming. \\ \cline{2-3}
~ & 1 & In embedded modes (\texttt{'l'}/\texttt{'i'}), slice from the generated code the statements that feed only output fields the wrapper never reads (the \texttt{\_location}/\texttt{\_size} metadata), so their constant index tables drop from the embedded data. Guarded by a generation-time correctness check; left unsliced on any uncertainty. \\ \hline
\end{longtable}
...
\texttt{output.JacobianStructure} - sparse ones and zeros defining the\\ Jacobian sparsity pattern
\end{tabular}\\ \hline
...
\emph{Note (v1.5 fork).} The five \texttt{adigatorGenFiles4*} commands documented in the following subsections (for \texttt{fsolve}, \texttt{fminunc}, \texttt{fmincon}, IPOPT, and GPOPS~II) are convenience wrappers carried over from upstream \emph{ADiGator}. They emit \textbf{host-only} derivative files (the classic runtime-\texttt{load}/global mechanism) and are \textbf{not at feature parity} with the rest of this fork: they do not support embedded code generation (\texttt{EMBED\_MODE}, MATLAB / Embedded Coder), the \texttt{PATH} option, the \texttt{nonzeros} output form, or reverse mode, and are not maintained or tested to the depth of the core generators. For embeddable derivatives use the core generators instead (Section~\ref{Sec:Embedded}); these wrappers are retained only for upstream compatibility.
...
(ADR-0025), so it is a slice of the same bytes the generator emits:
...
\texttt{examples/hessians/logsumexp/}\\
\texttt{examples/optimization/pipg/} (Hessian; uses the deprecated mode \texttt{'l'},\\ see Table \ref{Tab:Options})
\end{tabular}
\end{tabular}\\\\
...
\begin{itemize}
\item {\bf Nonzero-vector output (\texttt{DER\_OUTPUT}).} With \texttt{DER\_OUTPUT = 'nonzeros'} (Table \ref{Tab:Options}) a generator returns its {\em top-order} derivative's nonzero vector in the fixed pattern order, exporting the constant sparsity pattern once through the matching \texttt{output.*Locs}/\texttt{output.*Structure} companion, with no per-call dense allocation or scatter. This covers the {\bf Jacobian} (\texttt{output.JacobianLocs}, via \texttt{adigatorGenJacFile}) and the {\bf Hessian} (\texttt{output.HessianLocs}, via \texttt{adigatorGenHesFile}); on a Hessian file it flips only the Hessian output, the gradient companion staying dense. \texttt{JAC\_OUTPUT} is retained as a first-derivative-level-only alias.
\item {\bf Matrix-free reverse mode.} The transposed-Jacobian-vector product ($\bfJ^\top\bfv$) and the scalar-cost adjoint gradient are provided as the black box routines \texttt{adigatorGenJtVFile} and \texttt{adigatorGenRevGradFile}, documented in Section \ref{SubSec:ReverseBlackBox}. Within the embedded generator, the same adjoint gradient is produced as an {\bf embeddable} file by passing \texttt{DerType = 'gradient-reverse'} to \texttt{adigatorGenDerFile\_embedded}, which routes the adjoint through the classic/coderload/inline pipeline exactly like the forward modes.
\end{itemize}
...
The three embed modes return identical numbers, so the choice is about the generated artifact. In short: use inline \texttt{'i'} (the default for \texttt{adigatorGenDerFile\_embedded}) for embedded code generation --- it is self-contained (no \texttt{.mat}, no \texttt{load}, no globals) and code-generates under MATLAB Coder / Embedded Coder; use classic \texttt{'c'} for interactive/host use where the runtime \texttt{load} is convenient; coderload \texttt{'l'} is deprecated (Table \ref{Tab:Options}). For the derivative {\em form}, prefer \texttt{DER\_OUTPUT = 'nonzeros'} when a downstream solver assembles (or never forms) the matrix itself, and the matrix-free products (Section \ref{SubSec:OutputForms}) when only $\bfJ^\top\bfv$ or an adjoint gradient is needed.
...
\emph{Note (v1.5 fork).} Like the other \texttt{adigatorGenFiles4*} wrappers (Section~\ref{SubSec:SolverWrappers}), \texttt{adigatorGenFiles4gpops2} is inherited from upstream \emph{ADiGator} and is \textbf{not at feature parity} with this fork's embedded pipeline (host-only; no \texttt{EMBED\_MODE}/code generation/\texttt{PATH}/\texttt{nonzeros}/reverse mode). It is retained for upstream compatibility.\\
...
\subsection{Extended-Feature and Embedded Examples (Version 1.5 Fork)}
...
The code in \texttt{examples/optimization/pipg/} generates an embeddable Hessian of a small cone/gap function, $f = \bfw^\top\,\texttt{conefun}(\bfz) + \bfz^\top\,\texttt{setfun}(\bfz)$, with \texttt{adigatorGenDerFile\_embedded} (Section \ref{SubSec:GenDerEmbedded}), producing the wrapper triple \texttt{[H,G,f] = gapfun\_Hes(w,z)}. \emph{Note}: this example currently selects the deprecated coderload mode \texttt{'l'} (Table \ref{Tab:Options}); new code should use inline \texttt{'i'}.
