...
\emph{Embedded modes (v1.5 fork).} The allowances above (\texttt{global}/\texttt{load} treated as known auxiliary data) apply to the classic \texttt{'c'} mode. When generating an {\bf embeddable} file --- \texttt{adigatorGenDerFile\_embedded}, or any \texttt{EMBED\_MODE} other than \texttt{'c'} --- the differentiated function must be fully self-contained, so \texttt{global} variables, the \texttt{load} command, and cell arrays are {\bf not} supported: they are rejected at generation time with an actionable \texttt{adigator:embed:unsupportedConstruct} error naming the offending line. Pass such data as struct or numeric auxiliary inputs instead, pre-loading any data outside the function (ADR-0023).
