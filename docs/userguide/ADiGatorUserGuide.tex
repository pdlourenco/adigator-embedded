...
~ & \texttt{'l'} & Coder{\bf l}oad. The \texttt{.mat} is read through \texttt{coder.load} and wrapped in \texttt{coder.const}: loaded at runtime when interpreted, at compile-time into a persistent constant when compiled with MATLAB Coder. No global variables. \\ \cline{2-3}
...
{\bf JAC\_OUTPUT} & \texttt{'matrix'} & {\bf (v1.5 fork; \texttt{adigatorGenJacFile} only)} The wrapper projects the derivative into a dense/sparse Jacobian matrix on every call (default). \\ \cline{2-3}
~ & \texttt{'nonzeros'} & The wrapper returns the nonzero vector in the fixed pattern order, with the pattern exported once via \texttt{output.JacobianLocs}/\texttt{output.JacobianStructure}; no per-call allocation or scatter. \\ \hline
...
{\bf Input Info} & \begin{tabular}{l}
\texttt{DerType} - \texttt{'jacobian'}, \texttt{'gradient'}, or \texttt{'hessian'}\\
\texttt{UserFunName} - string name of file to be differentiated\\
...
\texttt{options} - (optional) options structure; see \texttt{EMBED\_MODE}, \texttt{PATH},\\ \texttt{DER\_LEVELS}, \texttt{SLIM\_EMBED} in Table \ref{Tab:Options}
\end{tabular}\\ \hline
{\bf Examples} & \begin{tabular}{l}
...
\begin{itemize}
\item {\bf Nonzero-vector Jacobian output.} With \texttt{JAC\_OUTPUT = 'nonzeros'} (Table \ref{Tab:Options}), \texttt{adigatorGenJacFile} returns the Jacobian nonzero vector in the fixed pattern order, exporting the constant sparsity pattern once through \texttt{output.JacobianLocs}/\texttt{output.JacobianStructure}, with no per-call dense allocation or scatter.
