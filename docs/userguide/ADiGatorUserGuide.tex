...
{\bf JAC\_OUTPUT} & \texttt{'matrix'} & {\bf (new in v2.0)} Legacy first-derivative-level-only alias of \texttt{DER\_OUTPUT} (\texttt{adigatorGenJacFile}): the Jacobian/gradient is projected to a matrix (default). Prefer \texttt{DER\_OUTPUT}. \\ \cline{2-3}
