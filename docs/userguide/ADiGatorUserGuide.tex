...
\end{tabular}\\ \hline
{\bf Examples} & \begin{tabular}{l}
\texttt{examples/optimization/pipg/} (Hessian, mode \texttt{'l'})\\
\texttt{examples/hessians/logsumexp/}
\end{tabular}
\end{tabular}\\\\
...
\item {\bf Transposed Jacobian-vector products.} \texttt{adigatorGenJtVFile} generates a file computing $\bfJ^\top \bfv$ for a runtime vector \texttt{v} in a single forward-plus-adjoint sweep, without forming the Jacobian.
\item {\bf Reverse-mode gradient.} \texttt{adigatorGenRevGradFile} generates a standalone reverse-mode (adjoint) gradient file for a scalar cost, computing the gradient in a fixed number of sweeps independent of the number of variables --- well suited to dense objective gradients (sums, log-sum-exp, least squares). See \texttt{examples/gradients/logsumexp/}. The same reverse gradient can be produced as an {\bf embeddable} file by passing \texttt{DerType = 'gradient-reverse'} to \texttt{adigatorGenDerFile\_embedded}, which routes the adjoint through the classic/coderload/inline pipeline exactly like the forward modes.
\end{itemize}
