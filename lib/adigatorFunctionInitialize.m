...
if ADIGATOR.OPTIONS.PREALLOCATE
  ...
  if ~isempty(ADIGATOR.FILE.PARENTID)
    FunctionInfo(ADIGATOR.FILE.PARENTID(end)).STRUCASGN = ADIGATOR.STRUCASGN;
  end
...
for Icount = 1:NUMinputs
  ...
  [Inputs{Icount},InVars,InVarStrs,InOps,NumInVars] =...
    FindInputVars(CurVar,CurVarStr,InVars,InVarStrs,InOps,NumInVars,PDflag,0);
  if isstruct(Inputs{Icount}) || iscell(Inputs{Icount})
    Inputs{Icount} = cadastruct(Inputs{Icount},CurVarStr,[],0);
  end
end
...
if ~ADIGATOR.RUNFLAG
  ...
  if FunAsLoopFlag || ~FunctionInfo(FunID).Iteration.CallCount
    ...
    if FunID == 1
      ...
      for Icount = 1:NumInVars
        ParLoc = ParNAMELOCS(InOps(Icount),3);
        if ParLoc == -Inf
          ADIGATOR.VARINFO.NAMELOCS(Icount,3) = -Inf;
        elseif ParLoc == Inf
          ADIGATOR.VARINFO.NAMELOCS(Icount,3) = Inf;
...
elseif ADIGATOR.OPTIONS.UNROLL && FunID > 1
  ...
  if ADIGATOR.EMPTYFLAG
    ...
    if ADIGATOR.RUNFLAG == 2 && FunID > 1
      ...
      for Icount = 1:NUMinputs
        InCallStr{Icount} = sprintf('cadainput%1.0d_%1.0d,',FunID,Icount);
      end
      ...
      if NUMoutputs == 1
        OutCallStr = sprintf('cadaoutput%1.0d_1',FunID);
      else
        OutCallStr = cell(1,NUMoutputs);
        for Ocount = 1:NUMoutputs
          OutCallStr{Ocount} = sprintf('cadaoutput%1.0d_%1.0d,',FunID,Ocount);
        end
    ...
    if ADIGATOR.DERNUMBER == 1 && any(ADIGATOR.VARINFO.NAMELOCS(1:NumInVars,2) < 0)
      ...
      for Icount = 1:NumInVars
        ...
        if HDfile < 0
          ...
          for Vcount = 1:ADIGATOR.NVAROFDIFF
            if ~isempty(xderiv(Vcount).nzlocs)
              oldderstr = cadadername(x.func.name,Vcount,Icount);
              fprintf(ADIGATOR.PRINT.FID,[xderiv(Vcount).name,' = ',oldderstr,';\n']);
            end
          end
          fprintf(ADIGATOR.PRINT.FID,[x.func.name,' = ',oldfunstr,';\n']);
...
function [CurVar,Vars,VarStrs,InOps,NumInVars] =...
  FindInputVars(CurVar,CurVarStr,Vars,VarStrs,InOps,NumInVars,PDflag,StructFlag)
% Find Input or Output Variables embedded within cells/structures
% StructFlag = 1 => in a cell/structure array
global ADIGATOR
structarrayflag = 0;
if isa(CurVar,'cadastruct')
  if ADIGATOR.FILE.FUNID
    StructID = cadaGetStructID(CurVar);
  else
    StructID = NumInVars+1;
  end
  structarrayflag = cadaIsArray(CurVar);
      ...
      for Jcount = 1:size(CurVar,2)
        for Scount = 1:length(StructNames)
          ...
          [CurVar(Icount,Jcount).(StructNames{Scount}),Vars,VarStrs,InOps,NumInVars] =...
            FindInputVars(NewVar,NewVarStr,Vars,VarStrs,InOps,NumInVars,PDflag,StructFlag);
        end
      end
    end
    StructFlag = StructFlag-1;
    if ~StructFlag
      CurVar = cadastruct(CurVar,CurVarStr,NumInVars,1);
      Vars{NumInVars}    = CurVar;
      VarStrs{NumInVars} = CurVarStr;
      InOps(NumInVars) = StructID;
    end
  else
    % -------------------------- Structure ------------------------------ %
    % These dont need special treatment like structure arrays do.
    for Scount = 1:length(StructNames)
...
%     if ~StructFlag
%       CurVar = cadastruct(CurVar,CurVarStr,[]);
%     end
  end
elseif iscell(CurVar)
  ...
  for Icount = 1:cRow
    for Jcount = 1:cCol
      ...
      NewVarStr = sprintf([CurVarStr,'{%1.0f,1.0f}'],Icount,Jcount);
...
if ~isempty(ADIGATOR.FILE.PARENTID)
  ...
  FunctionInfo(ParentID).STRUCASGN  = ADIGATOR.STRUCASGN;
