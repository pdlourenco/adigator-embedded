...
elseif ~strcmp(callerstr,'max') && ~strcmp(callerstr,'min')
  spflag = 0;
  if ~isempty(x.func.value)
    xtemp = logical(abs(x.func.value)); spflag = 1;
  ...
  if ~isempty(y.func.value)
    ytemp = logical(abs(y.func.value)); spflag = 1;
...
for Vcount = 1:NUMvod;
  ...
  if ~isempty(spdxind) && ~isempty(spdyind)
    ...
    if DPFLAG == 1
      ...
      if strcmp(callerstr,'power')
        TD2 = ['cada',NDstr,'td2'];
        fprintf(fid,[indent,TD2,' = ',getdzdx(Xstr,Ystr,DXstr,callerstr),';\n']);
        fprintf(fid,[indent,TD2,'((',Xstr,' == 0 & ',DXstr,' == 0) | ',Ystr,' == 0) = 0;\n']);
      ...
      if strcmp(callerstr,'mod')
        ...
        if yscalarflag
          ...
          TD2 = ['cada',NDstr,'td2'];
          fprintf(fid,[indent,TD2,' = ',getdzdy(Xstr,Ystr,DYstr,callerstr),';\n']);
          fprintf(fid,[indent,TD2,'(',Ystr,' == 0) = 0;\n']);
          fprintf(fid,[indent,DZYstr,' = ',DZYstr,' + ',TD2,';\n']);
        end
      else
  ...
  elseif ~isempty(spdxind)
    ...
    if DPFLAG == 1
      ...
      if strcmp(callerstr,'power') && ((~isempty(y.func.value) && any(y.func.value(:) < 1)) || isempty(y.func.value))
        fprintf(fid,[indent,derivstr,'((',Xstr,' == 0 & ',DXstr,' == 0) | ',Ystr,' == 0) = 0;\n']);
      end
    end
    ...
    if DPFLAG == 1
      ...
      if strcmp(callerstr,'mod')
        ...
        if yscalarflag
          ...
          fprintf(fid,[indent,derivstr,' = ',getdzdy(Xstr,Ystr,DYstr,callerstr),';\n']);
          fprintf(fid,[indent,derivstr,'(',Ystr,' == 0) = 0;\n']);
        end
      else
        % Get [dZ/dY]*dY and print assignment to dZ
        fprintf(fid,[indent,derivstr,' = ',getdzdy(Xstr,Ystr,DYstr,callerstr),';\n']);
