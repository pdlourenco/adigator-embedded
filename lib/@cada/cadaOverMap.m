...
%            ADIGATOR.VARINFO.OVERMAP.FOR(ADIGATOR.VARINFO.OVERMAP.FOR == RHSloc) = 0;
    ...
    if (cOverLoc || bOverLoc) && ~ADIGATOR.EMPTYFLAG
      ...
      [ForLength, ForIter, whileflag] = getForLength(InnerForLoc);
      if iOverLoc
        inVar = ADIGATORVARIABLESTORAGE.OVERMAP{iOverLoc};
      else
        inVar = x;
      end
      if (ForIter < ForLength || whileflag) && cOverLoc
        % Do CONTINUE Overmap
      ...
      if bOverLoc > 0 || (bOverLoc < 0 && (ForIter == ForLength || whileflag))
        % Do BREAK Overmap
...
function [ForLength, ForIter, whileflag] = getForLength(ForCount)
global ADIGATORFORDATA
