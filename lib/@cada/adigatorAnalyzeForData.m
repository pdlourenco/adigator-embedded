...
for Fcount = 2:length(ADIGATORFORDATA(FORCOUNT).FOR)
  ...
  if any(FORDEP & WHILEFLAGS)
    error('Cannot have iteration dependent organizational operations within ''while'' loops');
  end
  ...
  if nnz(FORDEP) > 0
    ...
    ADIGATORDATA.DATA.(sprintf('Index%1.0d',INDEXCOUNT)) = Inds;
    IndName = sprintf('Gator%1.0dData.Index%1.0d',...
...
for Rcount   = 1:length(ADIGATORFORDATA(FORCOUNT).SUBSREF)
  ...
  [FORLENGTHS,WHILEFLAGS] = GetForLengths(ADIGATORFORDATA,REFLOCS);
  ...
  if any(ySizes(1,1) ~= ySizes(1,:)) || any(ySizes(2,1)~=ySizes(2,:)) || ...
      any(xSizes(1,1) ~= xSizes(1,:)) || any(xSizes(2,1)~=xSizes(2,:))
    NonEmptyRefMap = logical(sum(reshape(REFINDS,NUMinds,prod(FORLENGTHS)),1));
  end
  ...
  if xOver.func.size(2) > 1 && ~any(isinf(xOver.func.size)) ...
    ...
    if ~ADIGATORFORDATA(REFLOCS(1,end)).SUBSREF(REFLOCS(2,end)).FLAGS(2)
      ...
      xSizeTemp(NonEmptyRefMap) = xSize;
  ...
  if yOver.func.size(2) > 1 && any(ySizes(1,:)~= yOver.func.size(1)) && ...
    ...
    ySizeTemp(:,NonEmptyRefMap) = ySizes;
  ...
  %[DerInds,emptycheck] = GetStructSubsrefInds(xOver,yOver,DEPINDS);
  %ADIGATORFORDATA(REFLOCS(1,end)).SUBSREF(REFLOCS(2,end)).FLAGS(2) = emptycheck;
...
for Scount   = 1:length(ADIGATORFORDATA(FORCOUNT).SUBSASGN)
  ...
  [FORLENGTHS,WHILEFLAGS] = GetForLengths(ADIGATORFORDATA,ASGNLOCS);
  if isempty(ASGNINDS)
  ...
  xCount   = ADIGATORFORDATA(ASGNLOCS(1,end)).SUBSASGN(ASGNLOCS(2,end)).VARS{1};
  xOverLoc = ADIGATOR.VARINFO.OVERMAP.FOR(xCount,1);
  ...
  if xOver.func.size(2) > 1 && any(xSizes(1,:)~=xOver.func.size(1)) && ...
    ...
    for Icount = 1:size(NewInds,2)
      if sum(NewInds(:,Icount))
        SScount = SScount+1;
        xRef = MaxRef(1:xSizes(1,SScount),1:xSizes(2,SScount));
        OldInd = NewInds(:,Icount);
        NewInds(logical(OldInd),Icount) = xRef(nonzeros(OldInd));
      end
    end
  ...
  for SScount = 1:2
    ...
    if any(bSize ~= bSize(1))
      ...
      if any(BSIZEDEP{SScount} & WHILEFLAGS)
        error('Cannot have iteration dependent organizational operations within ''while'' loops');
      end
      % Remove any unneeded sizes
...
for Scount   = 1:length(ADIGATORFORDATA(FORCOUNT).SPARSE)
  ...
  for SScount = 1:2
    ...
    if sum(xSize ~= xSize(1))
      ...
      if any(XSIZEDEP{SScount} & WHILEFLAGS)
        error('Cannot have iteration dependent organizational operations within ''while'' loops');
      end
      % Remove any unneeded sizes
...
for Ncount   = 1:length(ADIGATORFORDATA(FORCOUNT).NONZEROS)
  ...
  [FORLENGTHS,WHILEFLAGS] = GetForLengths(ADIGATORFORDATA,NZLOCS);
  if isempty(NZINDS)
...
for Hcount   = 1:length(ADIGATORFORDATA(FORCOUNT).HORZCAT)
  ...
  [FORLENGTHS,WHILEFLAGS] = GetForLengths(ADIGATORFORDATA,HLOCS);
  if isempty(HSIZES)
    for Fcount = 1:length(FORLENGTHS)
...
for VEcount  = 1:length(ADIGATORFORDATA(FORCOUNT).VERTCAT)
  ...
  [FORLENGTHS,WHILEFLAGS] = GetForLengths(ADIGATORFORDATA,VLOCS);
  if isempty(VSIZES)
    for Fcount = 1:length(FORLENGTHS)
...
for Tcount   = 1:length(ADIGATORFORDATA(FORCOUNT).TRANSPOSE)
  ...
  [FORLENGTHS,WHILEFLAGS] = GetForLengths(ADIGATORFORDATA,TRANLOCS);
  if isempty(TSIZES)
    for Fcount = 1:length(FORLENGTHS)
...
for REPcount = 1:length(ADIGATORFORDATA(FORCOUNT).REPMAT)
  ...
  [FORLENGTHS,WHILEFLAGS] = GetForLengths(ADIGATORFORDATA,REPLOCS);
  if isempty(RSIZES)
    for Fcount = 1:length(FORLENGTHS)
...
for REScount = 1:length(ADIGATORFORDATA(FORCOUNT).RESHAPE)
  ...
  [FORLENGTHS,WHILEFLAGS] = GetForLengths(ADIGATORFORDATA,RESLOCS);
  if isempty(RSIZES)
    for Fcount = 1:length(FORLENGTHS)
...
for SIZcount = 1:length(ADIGATORFORDATA(FORCOUNT).SIZE)
  ...
  [FORLENGTHS,WHILEFLAGS] = GetForLengths(ADIGATORFORDATA,SIZLOCS);
  if isempty(OUTSIZES)
    for Fcount = 1:length(FORLENGTHS)
...
%                            STRUCTREF                                    %
% ----------------------------------------------------------------------- %
for Rcount   = 1:length(ADIGATORFORDATA(FORCOUNT).STRUCTREF)
  % ---- Check for Input Reference Index Dependencies ------- %
  % We use the syntax y = x(inds);
  % Indices have been collected throughout 
  REFINDS    = ADIGATORFORDATA(FORCOUNT).STRUCTREF(Rcount).INDICES;
  REFLOCS    = ADIGATORFORDATA(FORCOUNT).STRUCTREF(Rcount).LOCS;
  [FORLENGTHS,WHILEFLAGS] = GetForLengths(ADIGATORFORDATA,REFLOCS);
  
  if isempty(REFINDS)
    % This call to subsref never fires for some reason
    for Fcount = 1:length(FORLENGTHS)
      ADIGATORFORDATA(REFLOCS(1,Fcount)).STRUCTREF(REFLOCS(2,Fcount))...
        .INDICES = [];
      ADIGATORFORDATA(REFLOCS(1,Fcount)).STRUCTREF(REFLOCS(2,Fcount))...
        .SIZES   = [];
    end
  ...
  % get overmapped x and y (y = x(inds))
  xOver = ADIGATORFORDATA(REFLOCS(1,end)).STRUCTREF(REFLOCS(2,end)).VARS{1};
  if isnumeric(xOver)
    if isempty(xOver)
      % xLoc will be empty whenever we are in a 2nd derivative and we are
      % referencing off of a set of FOR loop indices - dont worry about it,
  ...
  yOver = ADIGATORFORDATA(REFLOCS(1,end)).STRUCTREF(REFLOCS(2,end)).VARS{2};
  if isa(yOver,'cada')
    numrefvars = 1;
    structrefs = [];
    [DerInds,emptyderivchk,emptyfunchk,sizechk,yOver] = GetStructSubsrefInds(xOver,yOver,DEPINDS,structrefs);
  else
    [structrefs,numrefvars] = ParseStructForLoop({},struct('type',cell(0),'subs',cell(0)),...
      0,yOver);
    structrefs = structrefs(1:numrefvars);
    if numrefvars
      [DerInds,emptyderivchk,emptyfunchk,sizechk,yOver] = GetStructSubsrefInds(xOver,yOver,DEPINDS,structrefs);
    end
  end
  ADIGATORFORDATA(REFLOCS(1,end)).STRUCTREF(REFLOCS(2,end)).VARS{2} = yOver;
  
  
  
  % Pre-Allocate STRUCTREF .INDICES - We use .INDICES field to
  % store information on derivative indexing
  for Fcount = 1:length(FORLENGTHS)
    ADIGATORFORDATA(REFLOCS(1,Fcount)).STRUCTREF(REFLOCS(2,Fcount))...
      .INDICES = cell(NUMvod,numrefvars,3);
    ADIGATORFORDATA(REFLOCS(1,Fcount)).STRUCTREF(REFLOCS(2,Fcount))...
      .SIZES = cell(1,3);
  end
  if numrefvars > 0
    ADIGATORFORDATA(REFLOCS(1,end)).STRUCTREF(REFLOCS(2,end)).FLAGS{2} = structrefs;
    ADIGATORFORDATA(REFLOCS(1,end)).STRUCTREF(REFLOCS(2,end)).FLAGS{3} = emptyderivchk;
    ADIGATORFORDATA(REFLOCS(1,end)).STRUCTREF(REFLOCS(2,end)).FLAGS{4} = sizechk;
    for Vcount = 1:NUMvod
      for Scount = 1:numrefvars
        if ~isempty(DerInds{Vcount,Scount})
          DataNameCount = AssignReferenceNamesCat(DerInds{Vcount,Scount},REFDEP,REFLOCS,...
            FORLENGTHS,'STRUCTREF','INDICES',[Vcount Scount],DataNameCount);
        end
      end
    end
    if any(emptyfunchk(:))
      DataNameCount = AssignReferenceNames(emptyfunchk,REFDEP,REFLOCS,...
        FORLENGTHS,'STRUCTREF','SIZES',1,0,DataNameCount);
    end
  end
...
%                            STRUCTASGN                                   %
% ----------------------------------------------------------------------- %
for Scount   = 1:length(ADIGATORFORDATA(FORCOUNT).STRUCTASGN)
  % ---- Check for Input Assignment Index Dependencies ------- %
  % We use the syntax x(inds) = b;
  % Indices have been collected throughout 
  REFINDS    = ADIGATORFORDATA(FORCOUNT).STRUCTASGN(Scount).INDICES;
  REFLOCS    = ADIGATORFORDATA(FORCOUNT).STRUCTASGN(Scount).LOCS;
  [FORLENGTHS,WHILEFLAGS] = GetForLengths(ADIGATORFORDATA,REFLOCS);
  
  if isempty(REFINDS)
    % This call to subsasgn never fires for some reason
    for Fcount = 1:length(FORLENGTHS)
      ADIGATORFORDATA(REFLOCS(1,Fcount)).STRUCTASGN(REFLOCS(2,Fcount))...
        .INDICES = [];
      ADIGATORFORDATA(REFLOCS(1,Fcount)).STRUCTASGN(REFLOCS(2,Fcount))...
        .SIZES   = [];
    end
  ...
  % get overmapped x and b (x(inds) = b)
  xOver = ADIGATORFORDATA(REFLOCS(1,end)).STRUCTASGN(REFLOCS(2,end)).VARS{1};
  if isnumeric(xOver)
    if isempty(xOver)
      continue
    else
  ...
  bOver = ADIGATORFORDATA(REFLOCS(1,end)).STRUCTASGN(REFLOCS(2,end)).VARS{2};
  if isa(bOver,'cada')
    numrefvars = 1;
    structrefs = [];
    [DerInds,emptyderivchk,emptyfunchk,sizechk,bOver] = GetStructSubsrefInds(xOver,bOver,DEPINDS,structrefs);
  else
    [structrefs,numrefvars] = ParseStructForLoop({},struct('type',cell(0),'subs',cell(0)),...
      0,bOver);
    structrefs = structrefs(1:numrefvars);
  ...
  ADIGATORFORDATA(REFLOCS(1,end)).STRUCTASGN(REFLOCS(2,end)).VARS{2} = bOver;
  
  % Pre-Allocate STRUCTASGN .INDICES - We use .INDICES field to
  % store information on derivative indexing
  for Fcount = 1:length(FORLENGTHS)
    ADIGATORFORDATA(REFLOCS(1,Fcount)).STRUCTASGN(REFLOCS(2,Fcount))...
      .INDICES = cell(NUMvod,numrefvars,3);
  ...
  if numrefvars > 0
    ADIGATORFORDATA(REFLOCS(1,end)).STRUCTASGN(REFLOCS(2,end)).FLAGS{2} = structrefs;
    ADIGATORFORDATA(REFLOCS(1,end)).STRUCTASGN(REFLOCS(2,end)).FLAGS{3} = emptyderivchk;
    ADIGATORFORDATA(REFLOCS(1,end)).STRUCTASGN(REFLOCS(2,end)).FLAGS{4} = sizechk;
    for Vcount = 1:NUMvod
      for S2count = 1:numrefvars
        if ~isempty(DerInds{Vcount,S2count})
          DataNameCount = AssignReferenceNamesCat(DerInds{Vcount,S2count},REFDEP,REFLOCS,...
            FORLENGTHS,'STRUCTASGN','INDICES',[Vcount S2count],DataNameCount);
        end
      end
    end
    if any(emptyfunchk(:))
      DataNameCount = AssignReferenceNames(emptyfunchk,REFDEP,REFLOCS,...
        FORLENGTHS,'STRUCTASGN','SIZES',1,0,DataNameCount);
    end
  else
    ADIGATORFORDATA(REFLOCS(1,end)).STRUCTASGN(REFLOCS(2,end)).FLAGS{2} = [];
    ADIGATORFORDATA(REFLOCS(1,end)).STRUCTASGN(REFLOCS(2,end)).FLAGS{3} = [];
    ADIGATORFORDATA(REFLOCS(1,end)).STRUCTASGN(REFLOCS(2,end)).FLAGS{4} = [];
  end
end
return
end

function [FORLENGTHS,WHILEFLAGS] = GetForLengths(ADIGATORFORDATA,FunLocs)
NUMloops   = size(FunLocs,2);
FORLENGTHS = zeros(1,NUMloops);
WHILEFLAGS = zeros(1,NUMloops);
for Fcount = 1:NUMloops
  FORLENGTHS(Fcount) = ADIGATORFORDATA(FunLocs(1,Fcount)).MAXLENGTH;
  WHILEFLAGS(Fcount) = ADIGATORFORDATA(FunLocs(1,Fcount)).WHILEFLAG;
end
...
if isnumeric(Inds)
  ...
  % Indices are structure array
  NUMinds = size(Inds,2);
  NUMsubs = size(Inds,1);
  % Remove columns with all empty stuff
  nEmptyMap = true(NUMinds,1);
  for Jcount = 1:NUMinds
    nemptyflag = false;
    for Icount = 1:NUMsubs
      if ~isempty(Inds(Icount,Jcount).subs)
        nemptyflag = true;
        break
      end
    end
    nEmptyMap(Jcount) = nemptyflag;
  end
  NZinds = Inds(:,nEmptyMap);
  NUMnzinds = size(NZinds,2);
  if NUMnzinds == 1
    Dependency(1) = 0;
  else
    depflag = 0;
    for Jcount = 1:NUMnzinds-1
      if ~isequal(Inds(:,Jcount),Inds(:,Jcount+1))
        depflag = 1;
        break
      end
    end
    Dependency(1) = depflag;
  end
end
...
if NUMloops == 1
  if ~Dependency(1)
    ...
    if ~structflag
      ...
      nsubs = size(Inds,1);
      for Jcount = 1:size(Inds,2)
        for Icount = nsubs
          nemptyflag = false;
          if ~isempty(Inds(Icount,Jcount).type)
            NewInds = Inds(:,Jcount);
            nemptyflag = true;
            break
          end
...
if Dcount < NUMloops && sum(Dependency(Dcount:end)) < length(Dependency(Dcount:end))
  ...
  % somewhere
  if structflag
    NewInds = struct('type',cell(1,Lengths(Dcount)),'subs',cell(1,Lengths(Dcount)));
  else
...
global ADIGATOR
NUMvod = ADIGATOR.NVAROFDIFF;
% This is for structure subsref, y = x(i) - basically we have a different x on each
% iteration and we need to map xi into yOver - this is done recursively in
% the case that xi and yOver are structures
% This function is also called for structure assignments - in structure
% assignments we have x(i) = y.
% Note: the overmap is done such that xi should always belong to y.


xval = cadaGetStruct(x);
if isa(y,'cadastruct')
  [yval,yname,yid,yflag] = cadastructDecomp(y);
end
...
if isempty(structrefs)
  ...
  NUMstruct = length(structrefs);
end
DerInds = cell(NUMvod,NUMstruct);
emptyderivchk = false(NUMstruct,NUMvod);
for Scount = 1:NUMstruct
  if ~isempty(structrefs)
    y = subsref(yval,structrefs{Scount});
  end
  % Make sure overmaps are good..
  for J = 1:ForLength
    refind = inds(:,J);
    if isempty(refind(1).type)
      continue
    end
    xi = subsref(xval,refind);
    if ~isempty(structrefs)
      try
        xi = subsref(xi,structrefs{Scount});
      catch
        continue
      end
    end
    y = cadaUnionVars(y,xi);
    if ~isempty(structrefs)
      if length(structrefs{Scount}) > 1
        ytemp = subsref(yval,structrefs{Scount}(1:end-1));
      else
        ytemp = yval;
      end
      send = structrefs{Scount}(end);
      switch send.type
        case '.'
          ytemp.(send.subs) = y;
        case '{}'
      ...
      if length(structrefs{Scount}) > 1
        yval = subsasgn(yval,structrefs{Scount}(1:end-1),ytemp);
      else
        yval = ytemp;
      end
    end
  ...
  if DPFLAG
    for Vcount = 1:NUMvod
      if ~isempty(y.deriv(Vcount).nzlocs)
        ...
        % dy map with ones
        dyoveri = sparse(yrows,ycols,1:nzy,yMrow*yNcol,nv);
        dyoverilog = logical(dyoveri);
        % Pre-Allocate Derivative Indices
        Dinds = zeros(nzy,ForLength);
        for J = 1:ForLength
          refind = inds(:,J);
          if isempty(refind(1).type)
            continue
          ...
          if ~isempty(structrefs)
            try
              ...
              emptyderivchk(Scount) = true;
              continue
            end
          end
          if ~isa(xi,'cada') || isempty(xi.deriv(Vcount).nzlocs)
            emptyderivchk(Scount) = true;
            sameflag = 0;
            continue
          end
          xMrow = xi.func.size(1); xNcol = xi.func.size(2);
          if isinf(xMrow); xMrow = 1; elseif isinf(xNcol); xNcol = 1; end
          % get dx deriv pattern
          xrows = xi.deriv(Vcount).nzlocs(:,1);
          xcols = xi.deriv(Vcount).nzlocs(:,2);
          nzx   = length(xrows);
          if xMrow <= yMrow && yNcol > 1
            xref = yRef(1:xMrow,1:xNcol);
            xref = xref(:);
            xrows = xref(xrows);
          end
          % dx map with index
          dxi = sparse(xrows,xcols,1:nzx,yMrow*yNcol,nv);
          
          xindlocs = nonzeros(dxi(dyoverilog));
          yindlocs = full(dyoveri(logical(dxi) & dyoverilog));
          
          Dinds(yindlocs,J) = xindlocs;
          if nzx ~= nzy || length(xindlocs) ~= nzx
            sameflag = 0;
          end
        end
        if ~sameflag
          DerInds{Vcount,Scount} = Dinds;
        end
      end
    end
  end
end
sizechk      = false(NUMstruct,1);
emptyfunchk  = zeros(NUMstruct,ForLength);
for Scount = 1:NUMstruct
  if ~isempty(structrefs)
    y = subsref(yval,structrefs{Scount});
  end
  ...
  for I = 1:ForLength
    ...
    if ~isempty(structrefs)
      try
        ...
        emptyfunchk(Scount,I) = 1;
        sizechk(Scount) = true;
        continue
      end
    end
    if ~isa(xi,'cada') || ~isequal(xi.func.size,ysize)
      sizechk(Scount) = true;
    end
  end
end
...
if isa(x,'cada') && cadaCheckForDerivs(x)
  count = count+1;
  if length(refs) < count
    refs{count*2} = [];
  end
  refs{count} = s;
elseif isstruct(x)
  ...
  if numel(x) > 1
    ...
    for K = 1:length(Fnames)
      ...
      for I = 1:numel(x)
        s2(end-1).subs = {I};
        s2(end).subs = F;
        [refs,count] = ParseStructForLoop(refs,s2,count,x(I).(F));
      end
    end
  elseif ~isempty(x)
    s2(end+1).type = '.';
    for K = 1:length(Fnames)
      F = Fnames{K};
      s2(end).subs = F;
      [refs,count] = ParseStructForLoop(refs,s2,count,x.(F));
    end
  end
elseif iscell(x)
  s2 = s;
...
INDEXCOUNT = ADIGATORDATA.INDEXCOUNT + 1;
ADIGATORDATA.INDEXCOUNT = INDEXCOUNT;
ADIGATORDATA.DATA.(sprintf('Index%1.0d',INDEXCOUNT)) = Data;
DataName = sprintf('Gator%1.0dData.Index%1.0d',...
  ADIGATOR.DERNUMBER,INDEXCOUNT);
