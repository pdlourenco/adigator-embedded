classdef cada
  ...
  methods
    ...
    function y = norm(x,p)
      ...
      % below are valid) is anything with a singleton dimension OR an
      % unknown/variable-length dimension (ADiGator marks those Inf), OR a
      % zero/empty dimension. The zero case matters: ADiGator assigns size
      % [0 0] both to a genuinely empty array and, as a constructor
      ...
      isvec = any(xsize == 1) || any(isinf(xsize)) || any(xsize == 0);
