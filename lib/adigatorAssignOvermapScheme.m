      ...
      if ~FunAsLoopFlag
        ...
        for Ncount = 1:length(NameLocs)
          ...
          if ~isempty(VarCount) && LASTOCC(VarCount,1) >= Start
            % This variable is defined prior to loop, used within loop,
            % then changed within loop.
            SaveCount = SaveCount+1;
            SAVE.FOR(VarCount,1) = SaveCount;
            FORDATA(ForCount).PREVOVERMAP = [FORDATA(ForCount).PREVOVERMAP VarCount];
            OverLoc = OVERMAP.FOR(AllAsgnCounts(find(NAMELOCS(AllAsgnCounts,1)...
              ==NameLoc,1,'last')),1);
            OVERMAP.FOR(VarCount,2) = OverLoc;
          else
            % Want to check for variables which are subsasgn'd to in the
            % loop, but are not pre-allocated.
            VarCount = AllVarCounts(find(NAMELOCS(AllVarCounts,1) == NameLoc,1,'first'));
            if ~NAMELOCS(VarCount,3)
              %SaveCount = SaveCount+1;
              %SAVE.FOR(VarCount,1) = SaveCount;
              FORDATA(ForCount).PREVOVERMAP = [FORDATA(ForCount).PREVOVERMAP VarCount];
              %OverLoc = OVERMAP.FOR(AllAsgnCounts(find(NAMELOCS(AllAsgnCounts,1)...
...
for IFcount = 1:length(IFDATA)
  ...
  if ~OUTERLOC || (FunID > 1 && INNERLOC == 1)
    ...
    iForLoc  = INNERLOC;
    ForLoops = zeros(1,INNERLOC+1-OUTERLOC);
    ForLoops(1) = iForLoc;
  ...
  for Vcount = 1:nAsgnVarnameLocs
    ...
    if PrevFlag && INNERLOC && (FunID == 1 || INNERLOC > 1)
      ...
      % are embedded within
      for ForLoc = ForLoops
        if isempty(LastPrevOp) || (LastPrevOp < FORDATA(ForLoc).START)
          ...
          if ~isempty(LastLoopOp)
            ...
            elseif ~isempty(IfOvermap(Vcount).Prior)
              SAVE.IF(LastLoopOp,1) = SAVE.IF(IfOvermap(Vcount).Prior,1);
            else
              SaveCount = SaveCount+1;
              SAVE.IF(LastLoopOp,1) = SaveCount;
              IfOvermap(Vcount).Prior = LastLoopOp;
            end
    ...
    if OUTERLOC && ~UNROLL
      ...
      elseif PrevFlag && ~isempty(LastPrevOp) && LastPrevOp < LoopStart && OVERMAP.FOR(LastPrevOp,2)
      ...
      elseif PrevFlag && ~isempty(LastPrevOp) && LastPrevOp >= LoopStart
        % obj prior to cond block belongs to cond overmap and is defined
      ...
      if PrevFlag && ~isempty(LastPrevOp)
